...
命令历史与 Document Session 均按分镜 scope 隔离；一个脚本标签聚合其所有
分镜子文档的 dirty 状态，不会把一张图的保存或撤销应用到另一张图。
...
registerExtensionManifest({
  ...
  toolbarItems: [],
  activate({ kernel }) {
    const off = kernel.events.on('selection:changed', ({ current }) => {
...
- 主进程 `repositories/shotRepository.ts` 负责分镜文档读写。
- Asset、Folder、Project 均有独立 Repository
  与 script-shot 索引修复，Repository 的 list 不再隐式写盘。
...
节点脚本默认采用严格手动保存：切换分镜只保留内存缓存，用户按 `Ctrl+S`
后才批量写入所有已修改分镜。用户可在设置中启用自动保存并配置间隔秒数；
启用后由 `EditorDocumentService` 在文档停止变化达到该间隔时写入正式文件。

## 注册表

- Inspector：`src/renderer/src/inspector/registry.ts`
- Editor Extension：`src/renderer/src/editor/extensions/registry.ts`
...
- [x] 主进程已拆出 Asset、Shot、Project、Autosave 与原子 JSON Repository。
