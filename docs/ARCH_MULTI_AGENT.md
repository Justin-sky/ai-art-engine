...
>
> **M3 收官-F（编排记录节点级单跑）**：失败 / 中止的记录里可对单个节点右键「单跑此节点」——
> 纯函数 `shared/orchestratorNodeRerun.ts`（配单测）以「目标节点 + 被其级联阻塞的传递下游」为
> 受影响集（done 节点一律保留产出），仅当 job 为 failed/aborted、目标自身未完成且直接依赖已全部
