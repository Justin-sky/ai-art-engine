...
describe('readScriptTimelineFromGenParams / withScriptTimeline 往返', () => {
  it('clip 的 sourceNodeId / sourceNodeTitle 可持久化读取', () => {
    const clip: ScriptTimelineClip = {
      ...
      title: '镜头A',
      relativePath: 'videos/a.mp4',
      sourceNodeId: 'node-gen-1',
      sourceNodeTitle: '生成·分镜A',
      startSec: 0,
      durationSec: 4
    }
    ...
    expect(read.clips[0].sourceNodeId).toBe('node-gen-1')
    expect(read.clips[0].sourceNodeTitle).toBe('生成·分镜A')
  })
  ...
  it('按 timelineNodeId 分键时同样保留来源节点', () => {
    ...
    expect(read.clips[0].sourceNodeId).toBe('voice-node')
  })
  ...
  it('旧数据（无来源节点字段）兼容读取', () => {
    ...
    const read = readScriptTimelineFromGenParams(written)
    expect(read.clips[0].sourceNodeId).toBeUndefined()
  })
})
