...
describe('connect menu type filter', () => {
  ...
  it('director motion node only accepts panorama image and model inputs', () => {
    ...
    expect(inPorts.map((p) => [p.id, p.dataType])).toEqual([
      ['in-panorama', 'image'],
      ['in-model', 'model']
    ])
  })

  it('typeDef helpers: motion provides images and videos (panorama image + model input)', () => {
    ...
    expect(typeDefAcceptsDataType(motion, GraphPortType.model)).toBe(true)
