...
import {
  ...
  uploadProjectMediaToTos
} from '../src/main/services/tosUploadService'
...
describe('tosUploadService', () => {
  ...
  describe('uploadLocalFileToTos', () => {
    it('throws when file is missing', async () => {
      await expect(uploadLocalFileToTos(join(tmpDir, 'missing.mp4'))).rejects.toThrow(
        /待上传文件不存在/
      )
    ...
    it('throws when object storage is not configured', async () => {
      ...
      await expect(uploadLocalFileToTos(sampleFile)).rejects.toThrow(/未配置可用的对象存储/)
      expect(tosMocks.putObjectFromFile).not.toHaveBeenCalled()
    })
  ...
  describe('uploadProjectMediaToTos', () => {
    it('throws when project is not open', async () => {
      serviceMocks.isOpen.mockReturnValue(false)
      await expect(uploadProjectMediaToTos('a.mp4')).rejects.toThrow(/未打开工程/)
    })
    ...
    it('uploads from project root when open', async () => {
      ...
      const result = await uploadProjectMediaToTos('clip.mp4')
      expect(tosMocks.putObjectFromFile).toHaveBeenCalledWith(
  ...
  describe('deleteTosUploads', () => {
    it('returns empty logs and skips SDK for empty list', async () => {
      const logs = await deleteTosUploads([])
      expect(logs).toEqual([])
