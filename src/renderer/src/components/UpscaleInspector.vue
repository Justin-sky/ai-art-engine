<template>
  <div class="node-inspector" v-if="node">
    ...
    <p class="hint">{{ t('graph.inspector.upscale.hint') }}</p>
    ...
    <label>
      {{ t('graph.upscale.systemPrompt') }}
      <ExpandableTextarea
        :key="`sys-${node.id}`"
        v-model="systemPrompt"
        :title="t('graph.upscale.systemPrompt')"
        :rows="5"
...
const node = computed(() => {
  ...
  return current?.typeId === 'image.upscale' ? current : null
})
...
const typeLabel = computed(() => graphTypeLabel('image.upscale'))
const emptyPrompt = computed(() => t('graph.upscale.promptEmpty'))
...
function loadSystemPrompt(current: NonNullable<typeof node.value>): void {
  ...
  systemPrompt.value = resolveUpscaleSystemPrompt(
...
watch(locale, (next) => {
  ...
  if (!cur || isDefaultSystemPrompt(cur)) {
    systemPrompt.value = defaultUpscaleSystemPrompt(String(next))
  }
})

function isDefaultSystemPrompt(value: string): boolean {
  return value === DEFAULT_UPSCALE_SYSTEM_PROMPT_EN || value === DEFAULT_UPSCALE_SYSTEM_PROMPT_ZH
}
