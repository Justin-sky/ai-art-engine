  ...
  <div v-if="node" class="node-inspector">
    ...
    <label>
      ...
      <ExpandableTextarea
        ...
        :title="t('graph.expand.systemPrompt')"
        :rows="5"
...
import {
  DEFAULT_EXPAND_SYSTEM_PROMPT_EN,
  DEFAULT_EXPAND_SYSTEM_PROMPT_ZH,
  buildExpandPrompt,
...
const node = computed(() => {
  ...
  return current?.typeId === 'image.expand' ? current : null
})
...
const typeLabel = computed(() => graphTypeLabel('image.expand'))
const displayTitle = useNodeDisplayTitle(node, typeLabel)
const emptyPrompt = computed(() => t('graph.expand.promptEmpty'))
...
const mergedPrompt = computed(() => {
  ...
  if (!current) return ''
  return buildExpandPrompt(readImageExpandFromNode(current.params))
})
...
function loadSystemPrompt(current: NonNullable<typeof node.value>): void {
  ...
  systemPrompt.value = resolveExpandSystemPrompt(
...
watch(locale, (next) => {
  ...
  if (!cur || cur === DEFAULT_EXPAND_SYSTEM_PROMPT_EN || cur === DEFAULT_EXPAND_SYSTEM_PROMPT_ZH) {
    systemPrompt.value = defaultExpandSystemPrompt(String(next))
  }
})
