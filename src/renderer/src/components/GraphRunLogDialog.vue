  ...
  >
    ...
    <div class="log-layout">
      ...
      <section class="event-pane">
        ...
        <p v-if="!selectedSession" class="empty">
          {{ t('graph.logs.emptyEvents') }}
        </p>
        <p v-else-if="!filteredEvents.length" class="empty">
          {{ t('graph.logs.emptyFiltered') }}
        </p>
        <template v-else>
          <div ref="splitHostEl" class="split-host">
            ...
            <div class="detail-pane">
              ...
              <p v-if="!selectedEvent" class="empty detail-empty">
                {{ t('graph.logs.detailHint') }}
              </p>
              <template v-else>
                ...
                <p v-if="selectedEvent.message" class="detail-message">
                ...
                <template v-if="selectedApiCalls.length">
                  <div v-for="(call, index) in selectedApiCalls" :key="call.id" class="api-block">
                    <div class="api-head">
                    ...
                    <p v-if="call.error" class="error-text">
                      {{ call.error }}
                    </p>
                    ...
                    <pre v-if="call.response" class="code-block">{{
                      formatJson(call.response)
                    }}</pre>
                    <p v-else class="muted">
                      {{ t('graph.logs.apiResponseEmpty') }}
                    </p>
                  </div>
                </template>
...
const selectedEventHasPorts = computed(() => {
  ...
  return event.inputs != null || (!!event.outputs && Object.keys(event.outputs).length > 0)
})
...
function isPassthroughOutputType(typeId: string | undefined): boolean {
  ...
  return typeId.startsWith('output.') || typeId === 'world.table' || typeId === 'beat.table'
}
