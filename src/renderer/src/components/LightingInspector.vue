<template>
  ...
  >
    ...
    <p class="hint">
      {{ t('graph.inspector.lighting.hint') }}
    </p>
...
const outputPrompt = computed(() => {
  ...
  if (!current) return ''
  // 始终按当前选项重算，编辑面板实时写回时 Inspector 同步刷新
...
function loadSystemPrompt(current: NonNullable<typeof node.value>): void {
  ...
  systemPrompt.value = resolveLightingSystemPrompt(
...
watch(locale, (next) => {
  ...
  if (!cur || isDefaultSystemPrompt(cur)) {
    systemPrompt.value = defaultLightingSystemPrompt(String(next))
  }
})

function isDefaultSystemPrompt(value: string): boolean {
  return value === DEFAULT_LIGHTING_SYSTEM_PROMPT_EN || value === DEFAULT_LIGHTING_SYSTEM_PROMPT_ZH
}
