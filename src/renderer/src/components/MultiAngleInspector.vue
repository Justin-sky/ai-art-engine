...
const node = computed(() => {
  ...
  return current?.typeId === 'image.multiAngle' ? current : null
})
...
const typeLabel = computed(() => graphTypeLabel('image.multiAngle'))
const displayTitle = useNodeDisplayTitle(node, typeLabel)
const emptyPrompt = computed(() => t('graph.multiAngle.promptEmpty'))
...
const outputPrompt = computed(() => {
  ...
  if (!current) return ''
  // 始终按当前机位/面板选项重算，编辑面板实时写回时 Inspector 同步刷新
  return resolveMultiAngleOutputPrompt(
    readMultiAngleCameraFromNode(current.params),
    current.params.text ?? ''
  )
})
...
function loadSystemPrompt(current: NonNullable<typeof node.value>): void {
  ...
  systemPrompt.value = resolveMultiAngleSystemPrompt(
...
watch(locale, (next) => {
  ...
  if (!cur || isDefaultSystemPrompt(cur)) {
    systemPrompt.value = defaultMultiAngleSystemPrompt(String(next))
  }
})

function isDefaultSystemPrompt(value: string): boolean {
  return (
    value === DEFAULT_MULTI_ANGLE_SYSTEM_PROMPT_EN || value === DEFAULT_MULTI_ANGLE_SYSTEM_PROMPT_ZH
  )
}
