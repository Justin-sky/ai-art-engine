<template>
  <div class="beat-unit-editor-body">
    <div class="graph-row">
      <NodeGraphEditor
        v-if="activeBeatId"
        :key="`${beatAssetId}:${activeBeatId}`"
        ref="graphRef"
        class="unit-graph"
        :asset-id="beatAssetId"
        :beat-unit-id="activeBeatId"
        scope="beatUnit"
    ...
    <div class="units-section" :class="{ collapsed: !unitsExpanded }">
      <button
        ...
        :title="unitsExpanded ? t('beat.strip.collapse') : t('beat.strip.expand')"
        :aria-label="unitsExpanded ? t('beat.strip.collapse') : t('beat.strip.expand')"
      ...
      >
        <svg viewBox="0 0 16 16" width="12" height="12" aria-hidden="true">
          ...
          />
          <path v-else fill="currentColor" d="M3.2 6.2 8 11l4.8-4.8-.9-.9L8 9.2 4.1 5.3z" />
        </svg>
...
import BeatStrip from './BeatStrip.vue'
...
const stripRef = ref<InstanceType<typeof BeatStrip> | null>(null)
const unitsExpanded = ref(true)

const beatAssetId = computed(() => props.beatAssetId)
const activeBeatId = computed(() => workspace.activeBeatId)
