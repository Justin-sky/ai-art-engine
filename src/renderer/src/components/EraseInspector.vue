<template>
  <div class="node-inspector" v-if="node">
    ...
    <p class="hint">{{ t('graph.inspector.erase.hint') }}</p>
    ...
    <label>
      {{ t('graph.erase.systemPrompt') }}
      <ExpandableTextarea
        :key="`sys-${node.id}`"
        v-model="systemPrompt"
        :title="t('graph.erase.systemPrompt')"
        :rows="5"
...
import {
  DEFAULT_ERASE_SYSTEM_PROMPT_EN,
  DEFAULT_ERASE_SYSTEM_PROMPT_ZH,
...
const node = computed(() => {
  ...
  return current?.typeId === 'image.erase' ? current : null
})
...
const typeLabel = computed(() => graphTypeLabel('image.erase'))
const emptyPrompt = computed(() => t('graph.erase.promptEmpty'))
...
const mergedPrompt = computed(() => {
  ...
  if (!current) return ''
  return buildEraseUserPrompt(readImageEraseFromNode(current.params))
})
...
function loadSystemPrompt(current: NonNullable<typeof node.value>): void {
  ...
  systemPrompt.value = resolveEraseSystemPrompt(
...
watch(locale, (next) => {
  ...
  if (
    !cur ||
    cur === DEFAULT_ERASE_SYSTEM_PROMPT_EN ||
    cur === DEFAULT_ERASE_SYSTEM_PROMPT_ZH
  ) {
    systemPrompt.value = defaultEraseSystemPrompt(String(next))
  }
})
