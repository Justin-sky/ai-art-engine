...
export default {
  ...
  stage2d: {
    ...
    rigFromImage: '🤸 从图片反解姿势',
    rigFromImageTip: '选一张人像照片，自动把姿态关键点反解成当前骨骼的平面旋转',
    actionTitle: '动作试播',
    ...
    spineExportNote: '把挂到关节的可见部件层导出为 skeleton.json + .atlas + 部件 PNG（当前摆姿并入初始姿态，组件保持未翻转的原始方向）',
    spineExporting: '正在组装 Spine 骨架包…',
    spineNoAttach: '先在「层」页把部件图拖入画布，再回到「骨骼」页选中关节后点「绑定当前层到选中关节」',
    spineExportDone: '已写入 {path}（{count} 个部件页 + skeleton.json + .atlas），素材库已刷新',
    autoCut: '从整图自动拆件',
    autoCutHint: '把整图立绘沿骨骼关节切成部件层并自动挂点（需先在「骨骼」页建好/对齐人形骨架；拆件以绑定姿势为参考，切完摆姿复位，当前画布即原图）',
    ...
    autoCutDone: '已切成 {count} 个部件并自动挂到骨骼（到「骨骼」页摆姿验证，或直接导出 Spine 骨架包）',
    ...
    poseDialogTitle: '从图片反解起始姿势',
    poseDialogSubtitle: 'YOLO 单人姿态 → 当前骨骼（仅平面旋转；命名含 chest/shoulder/elbow/hip/knee… 才参与驱动）',
    poseSearch: '搜图片素材…',
    poseNoImages: '还没有图片素材，先在资源面板导入一张人像',
    posePickFirst: '左边选一张人像图，选好后自动检测',
    poseModel: '姿态模型',
    poseFlip: '镜像参考图（自拍/镜像素材）',
    poseDriveTorso: '驱动躯干',
    poseDetect: '检测图中人物',
    poseDetecting: '检测中…',
    poseNoPerson: '没检测到完整的人物骨架',
    posePeopleFound: '检测到 {count} 人，点击骨架可切换跟踪的人',
    posePerson: '人物',
    posePersonName: '人物 {n}',
    ...
    actionPreviewNoFrames: '动作还没有关键帧——在 2D 骨骼编辑器生成/编辑动作并重新「💾 存为动作资产」后即可循环试播',
    actionPreviewSingleFrame: '此资产只有单帧定格、没有可循环时长——在 2D 骨骼编辑器补足关键帧后再存即可试播',
    actionPreviewHint: '资产自带装配快照与动作帧：在任意 2D 骨骼节点编辑器的动作区「📥 载入素材库」选中它即可载回并精修',
    poseFileMissing: '素材文件缺失或打不开',
    poseDetectError: '检测失败，请换更清晰的人像试试',
    poseRigNoDrivable: '骨骼里没有可驱动的关节：请把关节命名为 chest/shoulder/elbow/wrist/hip/knee/ankle（带 L/R，如 shoulderL），或先用「人形模板」',
    poseSolveEmptyHint: '检测出的骨架会自动映射到当前骨骼：左侧绿块为已命中的驱动段',
    poseSolveEmpty: '没解出可用姿势（命中 {ok}/{total} 段）',
    poseApplied: '姿势已应用（命中 {ok}/{total} 段）',
    poseApply: '应用并关闭',
    poseCancel: '取消',
    poseFooterHint: '关键点置信度低的不驱动；命中段看下方状态块',
    segOk: '已命中',
    segNoJoint: '骨骼里没有该关节',
    segNoTip: '肢体缺少末端关节',
    segNoKp: '参考图缺关键点/置信度过低',
    segDeg: '关键点退化为点',
    segTorso: '躯干',
    segUpperarm: '上臂',
    segForearm: '前臂',
