...
export default {
  ...
  studio: {
    ...
    orchestrator: {
      ...
      nodeId: '节点 id',
      nodeAgent: '执行 Agent',
      nodeInstruction: '环节说明',
      nodeInstructionPlaceholder:
        '告诉该 Agent 要做什么、产出什么，例如：把上面的分镜逐镜扩展为绘图提示词（人物造型与全局参数保持一致）',
      run: '开始编排',
      submitting: '提交中…',
      undo: '撤销草稿修改',
      redo: '重做草稿修改',
      clear: '清空表单',
      noGoal: '请先填写总目标',
      noAgent: '当前没有可编排的 Agent（需至少一个非缺省角色，可在上方标签栏新建）',
      nodeLimit: '最多 {max} 个节点',
      canvasTitle: '依赖连线',
      canvasHint: '把节点 A 拖到节点 B 上：B 等 A 完成后执行（A→B）；点击连线删除该依赖',
      ...
      noTitle: '（未命名编排）',
      abortJob: '中止本次编排',
      abortJobConfirm: '确定中止该编排任务？已产出的节点结果会保留。',
      rerun: '续跑',
      rerunFailed: '重跑失败节点',
      rerunAborted: '继续运行',
      rerunConfirm: '已完成节点将保留，失败/未完成的节点会重新执行。确定续跑吗？',
      rerunFail: '续跑失败',
      reuseJob: '再来一轮',
      ...
      rerunNodeBlockedDeps: '该节点的前置环节尚未完成，请先处理失败的上游节点。',
      copy: '复制',
      copied: '已复制',
      copySummary: '复制汇总',
      summary: '最终汇总',
      ...
      planFailed: '自动拆解失败',
      planRepaired: '（已自动修正 {n} 处无效依赖引用）',
      valBadId: '节点「{id}」的 id 无效：仅支持字母、数字与 . _ -，且不超过 32 位',
      valDupId: '节点 id 重复：{id}，请先改名再提交',
      valNoAgent: '节点「{id}」未选择执行 Agent',
      valSelfDep: '节点「{id}」不能依赖自身',
      valMissingDep: '节点「{id}」依赖了未定义的节点：{dep}',
      valCycle: '节点「{id}」所在的依赖链路存在循环',
      unnamedNode: '（未命名节点）',
      issueMissingDep: '依赖了未定义的节点：{dep}',
      issueCycleDep: '与 {dep} 的依赖构成循环，需剔除其中一条',
      issuesCount: '{n} 处问题',
      fixNode: '一键修复',
      fixNodeTip: '自动修复本节点：剔除无效依赖、未选角色补默认、冲突/非法 id 自动重命名',
      stripDep: '剔除',
      stripDepTip: '剔除这条无效依赖',
      issueLocate: '点击定位到对应输入框',
      nodeFixed: '已修复该节点',
      planRemainIssues: '；另需处理 {n} 处节点问题（已就地标注，见下方节点列表）',
      viewFlow: '列表',
      viewGraph: '连线图',
