...
export default {
  ...
  studio: {
    ...
    orchestrator: {
      ...
      canvasHint:
        'Drag node A onto node B: B runs after A finishes (A→B). Click a link to remove it.',
      ...
      valMissingDep: 'Node "{id}" depends on an undefined node: {dep}',
      valCycle: 'The dependency chain of node "{id}" contains a cycle',
      unnamedNode: '(unnamed node)',
      issueMissingDep: 'Depends on an undefined node: {dep}',
      issueCycleDep: 'Dependency with {dep} is part of a cycle — remove one edge',
      issuesCount: '{n} issue(s)',
      fixNode: 'Fix all',
      fixNodeTip:
        'Auto-fix this node: drop invalid deps, fill a default role when none, auto-rename conflicting/invalid id',
      stripDep: 'Drop',
      stripDepTip: 'Drop this invalid dependency',
      issueLocate: 'Click to focus the related input',
      nodeFixed: 'Node fixed',
      planRemainIssues: '; {n} node issue(s) remain — annotated below',
      viewFlow: 'List',
      viewGraph: 'Graph',
