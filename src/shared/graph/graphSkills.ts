...
const SVG_MOTION_USAGE_ZH =
  ...
  '视觉自查（画了什么、动效对不对）直接看 svg.anim 烘出的 PNG 帧与 GIF 产物（task_status 会给出产物路径），不要用本机浏览器做无头截图：沙箱里 msedge / chrome 建不出内部 IPC 管道，会以 0x80000003 崩掉、在用户桌面上弹出系统错误框，而且工具调用照样失败。' // cjk-ok（dsh 技能用法文本：双语提示词数据）
...
const SVG_MOTION_USAGE_EN =
  ...
  ' For a visual self-check (what was drawn, whether the motion reads) inspect the PNG frames and the GIF that svg.anim bakes — task_status reports their paths — instead of taking headless screenshots with a local browser: inside the sandbox msedge / chrome cannot create their internal IPC pipe, so they abort with 0x80000003 and pop an out-of-app Windows error dialog, and the tool call fails anyway.'
