...
export const DEFAULT_UI_SPLIT_USER_PROMPT_EN =
  'Split the game system design into independent UI screens and output ONLY the bare JSON array required by the system prompt — no object wrapper, no markdown list. Each item is one screen with id, title, and a detailed image-generation prompt. Prompts must not specify concrete colors or visual style; both come from the style reference, and each prompt must end with the one-sentence style-lock clause telling the image model to strictly follow the style reference.'

export const DEFAULT_UI_SPLIT_USER_PROMPT_ZH =
  '请将策划案中的 UI 拆分为独立界面，并仅输出系统提示词要求的 JSON 数组本身——不要用对象包裹，不要用 markdown 列表。每一项对应一个界面，包含 id、title，以及可用于生图的详细提示词 prompt。提示词不要指定具体颜色或视觉风格，颜色与风格都交给风格参考图，且每条 prompt 必须以风格锁定语结尾，明确要求图像模型严格遵循风格参考图。'
