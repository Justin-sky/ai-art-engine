...
const TOOL_DEFS: McpToolDef[] = [
  ...
  },
  {
    ...
    title: '节点类型清单',
    description:
      '列出能被 graph_edit 添加到宿主资产子图的节点类型（与 graph_edit 的 node_upsert 校验同一白名单，含 2D 舞台 / 宫格切分 / 图标包等新节点）：typeId、名称、分类、端口（连线时 fromPort / toPort 用的 id）与可选默认参数。用于外部 Agent 自发现可建节点，避免用猜的 typeId 被 graph_edit 跳过。需要 2D 帧动画 / GIF 动图时查 `anim.2d`：一张按行列分格的序列图（sprite sheet）从 in 端口接入，按 animRows / animCols 逐格切成帧 PNG；animGifFps（1–24，默认 0 = 只切帧）> 0 时额外经 out-gif 端口产出 GIF 动图并落盘为工程资产；动作预设与自定义描述见 animPresetId / animInstruction。只读操作，无需打开工程。需要 SVG 矢量图 / 矢量动画（不是位图）时查 `svg.gen` 与 `svg.anim`：`svg.gen` 用文本模型把描述画成 SVG 源码并落盘为工程 .svg 资产（in 接文本指令、in-image 接参考图照图生矢量，自带 SVG 系统提示词）；要帧序列 / GIF 就把它接到 `svg.anim` 的 in 端口（端口类型 svg）烘焙：含 SMIL / CSS 动效出多帧 PNG（out / out-all）+ GIF（out-gif），静态 SVG 只出 1 帧、不产 GIF。关键参数：svgGenWidth / svgGenHeight（16–2048，默认 512）、svgGenBackground、svgFrames（2–60，默认 12）、svgDurationSec（0 = 自动探测动画周期，上限 30 秒），用 includeParams 查询。',
    inputSchema: {
      type: 'object',
      properties: {
