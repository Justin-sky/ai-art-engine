...
/** 单个用户消息里最多提交的图片数（与 dsh attachment 的每消息上限一致） */
const MAX_USER_IMAGES = 20

/**
 * 把任务文本中的 @图片路径 解析为多模态内容块：文本块与图片块按原文顺序交错，
 * 图片经 ctx.attachments（attachment-local 服务）规范化后以 image block 提交给模型，
 * 模型可直接看到图片内容；同时保留 @路径 文本，供模型在 generate_* 工具参数中引用同一文件。
 * 读取/校验失败的图片引用原样保留为文本，不阻塞任务。
 */
async function buildUserContent(task, attachments, io) {
  ...
  while ((m = refRe.exec(task)) !== null) {
    ...
    try {
      ...
      // 保留路径文本：模型可在工具调用中引用同一文件
      blocks.push({ type: 'text', text: '@' + prefix + raw })
    } catch (error) {
      io.stderr.write('[aiart-runner] image attach failed ' + raw + ': ' + (error instanceof Error ? error.message : String(error)) + '\n')
      blocks.push({ type: 'text', text: '@' + prefix + raw })
    }
  }
...
async function run(ctx, task, io) {
  ...
  try {
    // 任务文本中的 @图片路径 解析为多模态 image 块随请求提交（无图片时为纯文本）
    const userContent = await buildUserContent(task, ctx.get('attachments'), io)
