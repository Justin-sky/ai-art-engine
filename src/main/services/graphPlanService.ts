...
5. params 用节点声明的键：公共生成参数为 text, generateInstruction, generateSystemPrompt, generateAspectRatio, generateResolution, generateQuality, generateDuration, generateCount, generateSeed, generateSeedUseGlobal, generateFrameMode, generateAudio, notes, label, inputDataType；参考图参数为 styleReferenceSubject（"ui"＝UI 界面生图，允许复刻参考图界面语言）、styleImagesUseGlobal（false＝用节点本地风格图，须同时给 styleImages）、styleImages（条目须为 {"libraryId":"<风格库 id>"} 或 {"dataUrl":"data:..."}；不确定风格库 id 就别写）、characterRefs（条目须为 {"name":"角色名","imageUrl":"工程相对路径或 data URL"}，只写名字会被丢弃）；节点私有参数（如 anim.2d 的 animRows / animCols / animGifFps）按 typeId 语义填写。编造的键会被忽略。
6. 常见链：play.script / prompt.optimize → asset.image → asset.video；备注用 note.text；矢量图 / SVG 动画用 svg.gen（要帧序列或 GIF 再接 svg.anim，端口 svg → in；注意 image ≠ svg，位图端口接不进矢量口）。
7. 输出严格为此 JSON 形状：
