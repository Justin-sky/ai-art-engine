...
function notSupported(feature: string): Promise<never> {
  return Promise.reject(
    new Error(`Google（Gemini）提供商仅支持文本，暂未接入${feature}`)
  )
}

/** Google Gemini OpenAI 兼容层：仅对话模型（gemini-*），排除图片 / 视频 / 音频生成模型 */
function isGoogleTextModelId(modelId: string): boolean {
...
export const googleAdapter: ModelProviderAdapter = {
  ...
  async fetchCatalog(provider, modality) {
    ...
    try {
      ...
      throw new Error(`拉取 Google 模型列表失败: ${await readHttpError(err)}`)
    }
  },
