...
- **Safety rails** — `execute_blender_code` runs through a lexical guard (rejects `os` / `subprocess` / network modules, `open` / `__import__` escape chains, decorator / `class` / `type()` structural tricks; render / save / import-export bpy operators stay unrestricted because the tools would be useless otherwise). Screenshots go through a one-shot temp file and are returned to multimodal clients only — they never touch the project or the audit log.
...
- **3D model rigging / skeleton** — toggle "Generate rig" on a 3D node to ask the upstream for a rigged GLB: **Tripo** sends `rig=true`; **Meshy** sends `rigging=true` + `rig_type` (humanoid etc.) + `pose`; **Rodin (Hyper3D)** sends `rig=true` + `rig_type` + `rig_animation` (multipart; some tiers need `rig=true` for a rigged GLB). **Luma AI / Lux3D don't support rigging** — turning it on is rejected with a clear message rather than silently dropped.
