...
- **编排记录节点级单跑**：失败/中止的记录里右键单个节点「单跑此节点」，只重置该节点及其被级联阻塞的下游定点重跑（`shared/orchestratorNodeRerun.ts` 纯函数 + `orchestrator:rerun-node` 通道；done 节点保留产出、已完成节点不重复执行），从「整单续跑」细化到「定点重试一个环节」
- **编排记录整单重跑直接提交**：done 记录头部新增「整单重跑」主操作——跳过「再来一轮」的表单回填环节（不覆盖 / 不触碰当前草稿），确认后直接把该记录的 goal/节点原样交回 `runOrchestrator` 通道开启一轮全新 job（`renderer/utils/orchJobReuse.ts` 内 `orchestratorJobToRunInput` 纯转换，剥离节点运行态字段、不截断节点、配单测），与「再来一轮」形成「原样直跑 / 回填微调」互补
...
- **编排面板分区高度可调**：「新建编排」与「编排记录」之间新增可拖拽分隔条，上下拖动自由调整两区高度比例，双击恢复自动布局
- **编排画布尺寸交互（拖动调高 + 滚轮缩放）**：新建编排的「依赖连线」画布与编排记录的节点图下沿各加一条可拖拽分隔条（拖动调整可视高度，双击恢复默认，范围 80–720px）；Ctrl/⌘ + 滚轮缩放（0.5–2 倍，缩放比例按 job 独立记忆，普通滚轮仍滚动查看内容），非 100% 时在标题 / 提示行显示百分比按钮、点击恢复 100%；缩放以「占位层按缩放后尺寸撑开滚动区 + 画布 transform 缩放」实现，画布内部逻辑坐标不变，拖拽建边仅把指针坐标除以缩放系数，任意缩放下连线吸附与命中检测都对准卡片；同时移除画布 `min-width: 100%` 造成的 SVG 水平拉伸（内容窄于容器时连线与卡片错位）
- **编排节点卡片网格化**：新建编排的节点列表由「一行一个」的纵向堆叠改为自适应卡片网格（按面板可用宽度自动排 1–N 列，`minmax(184px, 1fr)`，窄面板自动回落单列），「添加节点」由底部整行按钮改为网格末尾的虚线「+」卡片、与节点卡片并排；卡片头部在窄列下自动换行（序号 / id / 角色下拉 / 删除互不挤压），指令输入限高 96px、超出内部滚动，长指令不再把卡片无限撑高——节点较多时纵向占用显著下降
