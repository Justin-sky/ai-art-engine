...
- **MCP `asset_import` 导入的素材现在会出现在对话流（导入登记为「素材导入」活动并逐条回报相对路径）**：会话流的资产卡只有一个来源——MCP 活动回报的相对路径，而导入此前完全不经活动链路（`asset_import` 是同步 handler，导完只广播一次 `ASSET_UPDATED`），于是 Agent 把工程外的 SVG / PSD / 图片收进资产库后素材库里有、对话流里一张卡都没有；与同一轮里的生成链路并排看（位图有卡、导入的无卡）很容易被判成「这一步没执行」。现在把导入接上 `runGenActivity`：`McpActivityTool` 增 `asset_import`（任务列表标签「素材导入」/「Import」，配一条蓝色 kind 样式），`runGenActivity` 的 `describe` 支持 `relativePaths` 多产物清单、并新增可选 `detail`（无 model 可展示的活动用它代替副标题，导入写首个文件名 + 总数），`asset_import` 按 `imported` 逐条回报工程内相对路径（`relativePath` 取首条以兼容单产物消费方；`skipped` 的文件不回报路径，跳过原因仍在返回值里逐条给出），执行日志的收尾文案按工具分「Imported」/「Generated」两种口径（导入不是模型产出）。活动标题与副标题的拼装抽到共享纯函数层 `assetImportActivityTitle` / `assetImportActivityDetail`（`src/shared/mcpAssetWrite.ts`）。导入的 SVG 无需额外处理：`ChatAssetPreview` 的图片白名单本就含 `svg`、矢量图预览走原文件链路，会话流侧零改动（`activityMediaPaths` 已支持多产物逐条出卡）。工具描述同步点明「导入会登记活动并回报相对路径」，让 Agent 知道这条路在对话里看得见。验证：`typecheck` / `lint` / `check:cjk` / `format:check` 通过，`mcpAssetWrite` 单测新增 4 项（标题数量、单件取文件名且正反斜杠都认、多件带总数、空列表不出现 undefined 副标题）

- **资产库目录 watchdog（往工程 `Assets/` 里直接放文件即自动入库，不必回界面手点导入）**：新增 `assetWatchService`，用 chokidar 监听当前工程的 `Assets/`，文件或目录变化经 500ms 防抖后自动补建缺失的 `.asset.json` 与文件夹元数据，前端收到 `FOLDERS_UPDATED` 后刷新素材库；`awaitWriteFinish` 等文件写完再扫描，避免读到写入中的半截文件；`.asset.json` / `.folder.json` / 隐藏文件 / `Cache` 目录一律忽略，防止自写元数据触发监听死循环；只重扫发生变更的目录（去重嵌套目录），收养到孤儿媒体后广播一次目录更新。打开 / 新建工程时启动、关闭工程时停止，扫描延后到下一个 tick、异步不阻塞主线程；依赖新增 chokidar
- **MCP `workflow_commit` 支持手写 plan（不必再先调 `workflow_plan`），节点参数改为按目标节点类型声明的键校验并回报 `warnings`**：此前提交计划必须先经 `workflow_plan` 生成，Agent 想直接提交自己编排好的 plan 要多绕一次规划调用。更关键的是参数校验口径：节点 `params` 此前不校验键，编造的键（如把 `animGifFps` 写成 `gifFps`）被静默丢弃、任务照旧「成功」但产物不对；现按目标节点类型声明的键校验，未声明的键一律列入返回的 `warnings`，图规划系统提示同步点明公共生成参数与节点私有参数（如 `anim.2d` 的 `animRows` / `animCols` / `animGifFps`）。对话流的工具事件新增 `args`（原始参数 JSON，截断 4000 字符），任务清单把参数解析成键值对展示 Agent 实际传了什么，`detail` 摘要改为优先取 `path` / `query` 等短键（不再把 `write` 的长 `content` 当摘要）。新增 2 项单测（保留 `anim.2d` 私有参数 / 未声明键出告警）
...
- **资产库窗口支持多选文件 / 多选目录拖拽到其它目录；后端新增 `folder_move` IPC + MCP 工具（磁盘搬移 + descendant asset.relativePath 写回 + 成环检测 + 同名追加 " 2" / " 3"）**：用户反馈「资产管理窗口中现在不支持多选拖动文件或目录到其它目录」。现状盘点：左侧目录树行没 `draggable`、没 folder MIME、目录移动后端零链路；多选资产虽然 `onDragStart` 写入了 `STUDIO_ASSET_IDS_DRAG_MIME`，但 `onDropToFolder` 用串行 `for await` 逐个 move、失败单独弹窗、性能差。本轮四件套：① 后端（`src/main/repositories/assetTreeStore.ts` 新增 `moveFolderBetweenFolders` + `relocateDir`——磁盘 `renameSync` + 跨卷 `cpSync + rmSync` 回退 + 名字冲突追加 " 2" / " 3" + 写回 `.folder.json`；`folderRepository.move` 重 scan 后 `writeAssetToTree` 写回所有 descendant asset 的 `.asset.json` relativePath；`projectService.moveFolder` 用 `collectFolderSubtreeIds` 做成环检测，新增 `MAIN_ERRORS.folderCycle`）。② IPC + MCP（`FOLDER_MOVE` channel + `MoveFolderInput` + `window.studio.moveFolder` + handler + MCP `folder_move` tool，与现有 `folder_create` 不同**不 broadcast** `FOLDERS_UPDATED`，保持与 `folder_rename` / `folder_delete` 一致——renderer 自己 `refreshFolders + refreshAssets`）。③ 前端（`STUDIO_FOLDER_IDS_DRAG_MIME` 常量；目录树行加 `:draggable` + `@dragstart.stop="onFolderDragStart"`，资产库根目录不可拖；`hasAssetMove` → `hasInternalDrag` 合并检测；新增 `dragKind` 区分；`onFolderDragOver` / `onCurrentFolderDragOver` 用 `dragKind` 替代 `hasInternalDrag`；`onDropToFolder` 拆 asset / folder 分支：asset 走 `moveAssetsToFolder` `Promise.allSettled` 并发 + 局部 splice + 失败聚合弹窗，folder 走 `moveFoldersToFolder` 自指拒绝 + 子孙跳过 + 并发 + 整体 `refreshFolders + refreshAssets` + 失败聚合弹窗；删内部 `moveAssetToFolder`）。④ i18n 新增 4 条（zh-CN + en-US）。验证：`tsc` / `eslint`（AssetBrowser.vue 仅既存 `vue/max-attributes-per-line` warning）/ `format:check` 全绿；`check:cjk` 无新增违规；全量 279 个测试文件 + 新增 `tests/folderMove.test.ts`（7 项）共 2096 项单测全过

- **`asset_import` 改收口为「拒工程内任意素材」（不再仅拦 `Cache/` / `Output/`，连 `Assets/` 已入库资产也拒）；`cacheOnlyGenExtraParams` 再抹顶层 `outputDir` / `folderId` 兜底 harness 缓存旧 schema 的字段泄漏**：上一轮改完后真机复测——做图生图（参考图 `Assets/Advert/...`）仍被自动入库到 `Assets/Advert/`。但静态分析确认 `generate_image` 的 handler 字面量构造 input 时**不读**顶层 `args.outputDir`、`extraParams.outputDir` 也被 `cacheOnlyGenExtraParams` 剥了，按理 `input.outputDir === undefined`、`attachExternalGeneratedFile` 应走默认 `Cache/Images`——产物直接落 `Assets/Advert/` 与代码逻辑矛盾。说明真正入库路径不在本代码段：要么 Agent 仍按 harness 缓存的旧 schema 描述调（harness 未重启且不严格校验 inputSchema），要么 Agent 换路（用 shell 工具复制 + 写 `.asset.json` 绕过 MCP）。无论哪条路，**两层兜底都生效**：**① `asset_import` 收口为「工程内任意素材」**——把 `isProjectGeneratedOutputPath` 重命名为 `isProjectInternalPath`（`src/shared/mcpAssetWrite.ts`），语义从「仅判 Cache/Output」扩到「工程内任意路径都拒」（包括 `Assets/` 已入库资产——再 import 会再复制一份为新文件名、对话流再出一张卡），实现简化（不再依赖 `generatedOutputDirNames` 列表，正反斜杠归一、大小写不敏感、相对路径按工程内路径理解三件不变），`cacheOutputDir` 形参改 `_cacheOutputDir` 保留 API 兼容；**文案更新**为「工程内任何素材（含 `Cache/` 与 `Output/` 的生成产物、`Assets/` 下的已入库资产）都不该走本工具」，区分两类工程内素材并指向两类正路（生成产物 → 「保存到资产库」按钮、已入库资产 → `asset_list` / `asset_read` 引用、不重导）。**② `cacheOnlyGenExtraParams` 加抹顶层**——就地 `delete args.outputDir; delete args.folderId`，杜绝 harness 缓存旧 schema 仍透传顶层字段的路径（`GenerateImageInput` 等类型允许 outputDir 存在，input 字面量构造时若不慎展开就会泄露，mutate args 副本从源头堵住）。验证：`typecheck` / `lint` / `check:cjk`（存量 2 处不变）/ `format:check` 通过，全量 279 文件 2100 项单测通过（`mcpAssetWrite.test.ts` 用例改写覆盖工程内任意路径拦截、相对路径按工程内理解、工程外放行；`mcpGenToolSchema.test.ts` 函数名与文案描述同步更新）

- **对话生成产物彻底不再「自动入库」：`asset_import` 拒收工程内 `Cache/` 与 `Output/` 下的文件，`generate_speech` / `generate_music` 补齐 Cache-only，`extraParams` 夹带 `outputDir` / `folderId` 的隐通道一并堵死**：真机实测的现象是「对话里已经出了一张产物卡（右下角带『保存到资产库』按钮），紧接着资产库里又多出一份同名文件、对话流末尾再冒一张重复卡」——例如 `Cache/Images/奶粉广告·双胞胎电商海报.png` 生成后，`Assets/Advert/奶粉广告·双胞胎电商海报.png` 又出现一份（字节数与 mtime 完全相同，是复制来的），Agent 还会回复「已生成入库」。排查后确认这是**三条各自独立、都能把同一份产物送进资产库**的路径，此前只堵了其中一部分：① `generate_image` / `generate_video` / `generate_model3d` 早先的 settle 钩子会在生成后直接把资产登记进库（已于上一轮改为 `settleAssetFolder` 位置传 `undefined`、`inputSchema` 去掉 `outputDir` / `folderId`），但 **Agent 仍能自己绕路**——它拿到 `Cache/Images/xxx.png` 的相对路径后拼出绝对路径调 `asset_import`，而该工具的本意是「收编工程外的本机素材」，对工程内文件照收不误，于是产物被再复制一份进 `Assets/`（旁挂 `.asset.json` 里的 `genParams.mediaRole: 'reference'` 就是这条导入链路写的标记，也是本次定位的决定性证据）；② `generate_speech` / `generate_music` 的 `inputSchema` 还留着 `outputDir` / `folderId`——`outputDir` 指向 `Assets/` 时 `attachExternalGeneratedFile` 会直接登记资产，`folderId` 更会经 settle 钩子把文件搬进资产库文件夹；③ `extraParams` 是自由对象，能把这两个键夹带进来（展开后恰好命中同一条入库分支），`inputSchema` 收窄拦不住它。本轮把三条路一起封掉：**① `asset_import` 拒收工程内生成产物**——新增共享纯函数 `isProjectGeneratedOutputPath(filePath, projectRoot, cacheOutputDir?)`（`src/shared/mcpAssetWrite.ts`：正反斜杠归一、大小写不敏感、工程根带不带尾斜杠都认、自定义缓存根与 `Output/` 由 `SCANNED_OUTPUT_DIRS` 覆盖、工程外的同名 `Cache/` 目录不误伤、工程根自身与空路径不算），命中即整单拒绝并抛 `projectGeneratedOutputImportError`，文案列出被拒文件、讲清原因并指明正确入口（「让用户在对话产物卡上点『保存到资产库』按钮」），免得 Agent 换个写法反复重试；**② 五个生成工具统一 Cache-only**——`generate_speech` / `generate_music` 的 `inputSchema` 去掉 `outputDir` / `folderId`、handler 不再传 settle 钩子（连带删掉已无调用者的 `settleAssetFolder` / `applyAssetFolder`），描述改为「落 `Cache/Voices` / `Cache/Music`，要入库由用户点按钮」；**③ 堵住透传夹带**——新增 `cacheOnlyGenExtraParams`（在 `extraParamsOf` 之上再 `delete outputDir / folderId`），五个生成工具一律改用它。渲染层兜底继续保留：对话流仍按**内容指纹**（文件总字节数 + 前 8KB 的 djb2，`src/renderer/src/features/media/chatAssetFingerprint.ts`，>50MB 跳过、失败回退 null 不去重）对近 20 张卡去重，即便产物经其它链路（`asset_qc_fix` 的 `Assets/QC/`、`ui_kit_extract` 等）再次进入会话也不会出重复卡。文档同步 `docs/MCP.md` 的 `asset_import` 说明、`generate_*` 落盘口径与 `extraParams` 段落。验证：`typecheck` / `lint` / `check:cjk`（新增文案已按 `cjk-ok` 惯例豁免）通过，全量 279 文件 2102 项单测通过（`mcpAssetWrite` 新增 10 项：产物目录判定 8 项含自定义缓存根 / 兄弟目录不误伤 / 工程外放行、拒收文案 2 项；`mcpGenToolSchema` 由 5 项扩到 9 项：覆盖 `generate_speech` / `generate_music` 的 schema 与 settle、`cacheOnlyGenExtraParams` 全量替换、`asset_import` 拦截与描述口径）

- **画布右键菜单新增「影视」父级分组，把「剧本 / 剧集 / 世界元素 / 场」收进二级菜单（菜单从两层变三层）**：这四组此前与「图片」「视频」等并列平铺在根菜单上（`CONTEXT_MENU_RESOURCE_GROUPS` 是一维扁平表），根菜单因此有十七个分组入口、其中四个只服务于影视叙事链路。现在新增「影视」父级：根菜单只剩它一个入口，悬浮后弹出「剧本 → 剧集 → 世界元素 → 场」四个二级分组，二级分组再悬浮出一层节点面板（`asset.screenplay`；`episode.anchorSelect` / `episode.cellSelect` 与五个剧集 Agent 预设；`asset.world` / `world.extract` / `world.table` / `world.gen`；`asset.beat` / `beat.select` / `beat.split` / `beat.table` / `beat.gen` / `beat.unitGen` / `beat.unitRef`）。**父级只作入口、自身不放节点**（`items` 为空），四个子分组的标签、图标与组内节点一字未改，组内节点仍按名称排序；二级分组顺序固定为声明顺序（剧本 → 剧集 → 世界元素 → 场，即叙事流水线顺序），不参与名称排序，根菜单里「影视」与其余分组一起按名称排。交互与一级完全一致：悬停展开、点击固定、再点收起、未固定时移出即收起，三级面板同样做贴边翻转（新抽 `measureSubmenuFlip`，一级 / 三级共用；两层面板分别带 `ctx-root-panel` / `ctx-nested-panel` 类以便各量各的），父面板与子面板的「桥接条」只保留在一级（三级面板紧贴父面板没有空隙，留着反而会在父面板右缘挡出一条 10px 死区）。切换父分组 / 收起父面板 / 关闭菜单都会清掉已展开的二级项。判定「哪些分组收进影视」的纯逻辑抽到 `src/renderer/src/features/graph/contextMenuGroups.ts`（`nestContextMenuResourceGroups`）：四个子分组全部在当前作用域不可添加时父级不出现，只命中部分时父级下只挂存在的分组。新增文案 `graph.context.groups.filmTv`（中「影视」/ 英「Film & TV」）。验证：`typecheck`（node + web，模板随 prettier 重排后再验一遍）/ `lint`（`check:cjk` 0 违规）/ `format:check` 通过，全量 276 文件 2068 项单测通过（新增 `contextMenuGroups` 7 项：四组从根菜单摘出、二级顺序不随输入顺序变、子分组清单与图标原样保留、无关分组不动、部分落空、四个全落空不出现父级、空清单不产生父级）

- **CI 工作流里最后一批 `node20` 的 action 全部升到 Node 24 运行时（`softprops/action-gh-release` v2 → v3、`actions/configure-pages` v5 → v6、`actions/upload-pages-artifact` v3 → v5、`actions/deploy-pages` v4 → v5）**：GitHub Actions 自 2025 年 9 月起弃用 Node 20 运行时，仍以 `node20` 声明的 action 会被 runner 强制在 Node 24 上执行并逐条打出告警（6.2.1 的 Release 作业里 `action-gh-release@v2` 就是这条）。四个引用逐个升大版本：`action-gh-release@v3` 唯一的破坏性变更就是运行时 20 → 24（inputs 与 v2 完全一致，`tag_name` / `name` / `draft` / `prerelease` / `generate_release_notes` / `files` 六个入参沿用，发版产物的形态不变）；Pages 三件套必须成组升级——`configure-pages@v6` 与 `deploy-pages@v5` 同为 node24 运行时，`upload-pages-artifact@v5` 虽是 composite 但内部固定的 `upload-artifact` 由 v4（node20）换成 v7（node24），顺带把 v4 引入的「默认不打包隐藏文件」一并继承（`website/` 无点文件，站点内容不受影响）。至此仓库引用的 action 全部落在 Node 24 线，`release.yml` / `ci.yml` / `deploy-website.yml` 三个工作流不再有弃用告警

- **对话产物卡从「一文件一卡」改为「同一批次一卡」（同一轮连排多张几乎相同的图不再刷屏）**：每轮结束的产物扫盘此前把本轮落盘的媒体逐条出卡，而同一次跑动常在几十毫秒内连出多份文件——`svg.gen` 的矢量源落 `{资产名}_{节点名}_{时间戳}.svg`、`svg.anim` 把同一份矢量烘焙成 `..._烘焙预览 PNG_....png`、多帧烘焙再叠 `_1/_2.png` 与 `.gif`，`persistGraphRunOutputs` 里的 `materializeNodePreviewParams` 还会为节点参数预览物化一份 `node-<节点id>_param-preview.svg`（与矢量源**逐字节相同**，实测 6 组 md5 全等），于是一轮连改 6 个版本的人像就一口气出 19 张卡、其中每版 3 张是同一张图（真机会话数据实测：19 条 `asset:scan:*` 卡时间戳同为扫盘那一刻、路径 19 条两两不同——去重本身没坏，是「同源多份副本」被当成了 19 个产物；按落盘时刻核对这批文件（同一版本的矢量源 / 烘焙位图 / 参数预览副本相隔 120~~320 ms，版本之间相隔 3~~7 分钟），新口径下聚成 6 张卡）。现在 `src/shared/outputScan.ts` 新增 `groupRoundOutputs`，把筛选结果按**同一批次**聚合成卡：顺序归并，两侧都带资产名前缀时必须前缀相同（`generatedAssetKeyOf` 从生成媒体名里剥出资产名，故矢量源 / 烘焙位图 / 帧序列认得出是一批）且间隔 ≤ 10s，任一侧没有前缀（参数预览这类附带产物）时只要求间隔 ≤ 2s，间距拉开即开下一张卡；一张卡的代表文件取组内最早写入的一条（卡片预览与默认保存名用它），其余折叠在卡内，卡上「含 N 个产物」角标展开成员清单（默认收起，避免又变成一列重复图），**每份产物仍可单独保存到资产库**（已保存状态与保存弹窗改按产物路径判定，不再按卡 key）；单卡最多折叠 12 份、单轮最多 24 张卡、参与聚合的文件最多 120 个，被挡下的部分照旧只提示数量。验证：`typecheck` / `lint` / 全量 2006 项单测通过（新增 12 项：同源归并、代表文件取最早、参数预览紧窗口挂载与超窗独立、跨资产分批、带序号帧序列、同源超窗、成员上限与卡上限的计数、聚合键解析边界）

- **代码风格一次性收口到 Prettier 并接进 CI**：全仓 `prettier --write` 收口（行尾统一 LF、同步 CJK 白名单形态），CI 新增 `format:check` 步骤，该格式化提交登记进 `.git-blame-ignore-revs`，避免污染 `git blame`（本次收口同时把 Vue 模板里的多语句内联处理器拆坏，见 Fixed 第一条）

- **SVG 生成提示词与技能文本收口到「动效只用 SMIL」（此前提示词主动推荐了逐帧烘焙根本不求值的 CSS `@keyframes` 与 `<animateMotion>`）**：`svg.gen` 的系统提示词与 `svg.motion` 技能的生成指令模板把可选动效写成「`<animate>` / `<animateTransform>` / `<animateMotion>` 或写在 `<style>` 里的 CSS `@keyframes`」，而下游逐帧烘焙的求值范围只由 `renderSvgFrames` 的 `ANIMATION_SELECTOR`（`animate` / `animateTransform` / `set`）决定——CSS `@keyframes` 根本没有 DOM 动画元素可收集，`<animateMotion>` 则因需要 path 求长被显式置为 `spec: null`（保留原值），于是「照着提示词写」与「能不能烘出 GIF」正好相反：模型越听话地写 CSS 动画或 `animateMotion`，`svg.anim` 探到的动画周期越可能为 0，最后只出 1 帧、不产 GIF（ROADMAP 里 CSS `@keyframes` 与 `<animateMotion>` 本就是列明的待办项），而用户看到的只是「说好的动画没动」。本轮把口径统一为「只留可求值的 SMIL」（**不改任何运行时链路**，`svgTimeline` / `renderSvgFrames` 一行未动）：① `resolveSvgGenSystemPrompt` 的中英系统提示词改为「动效只用 SMIL（`<animate>` / `<animateTransform>` / `<set>`）并做成可无缝循环的完整周期，不要用 CSS `@keyframes`、也不要用 `<animateMotion>`」，并点明原因是逐帧烘焙只求值这一类属性动画、其余会被当作静态图；② GraphSkill `svg.motion` 的中英生成指令模板同口径改写（该模板会随技能插值进 `svg.gen` 的 `generateInstruction`，是最靠近模型的一层）；③ 描述性文案里「含 SMIL / CSS 动效时逐帧烘焙」这类把 CSS 与 SMIL 并列的写法全部收敛为「含 SMIL 动效」（`svg.anim` 节点描述、`graph_node_types` 工具描述、`graph.svgAnim.inspectorHint` 中英），并顺手补上「动效须是 SVG 自身的 `<animate>` / `<animateTransform>` / `<set>`」这句判据——教程脚本的避坑清单与 ROADMAP 的待办条目保持原样（前者本就是这个口径，后者记录的是真实待办）
...
- **修复把 AI 对话（或任意编辑器页签）拖到资产 / 参数下方后松手，落点半屏灰色预览残留在面板上、看着像「面板没移动成功」**：dockview 对「同一个 dock 内」的面板拖动走内部 move，只触发 `onDidMovePanel`；`onDidDrop` 只服务跨 dockview / paneview 的拖放（`getPanelData()` 有值时走 `_onMove`，无值才 fire `onDidDrop`）。而资产 / 参数组的落点用的是锚点覆盖层（`.dv-drop-target-container`，暗色主题底 `--dv-drag-over-background-color: rgba(83,89,93,.5)`），只有真正走到 drop 才有一次 `overrideTarget.clear()`——面板侧的收尾 `handleSidePanelMoved` 此前对非侧栏 id 直接 `return`（它只为「资产 / 参数互拖叠放」服务），于是 `chat` 以及任何编辑器页签拖到资产 / 参数上时，这层半透明预览没人清，松手后就留在落位好的面板上，看到的是「资产下半屏变成一块灰色区域、对话没跑到资产下面」。现在收尾拆成与面板类型无关的 `clearDockDropArtifacts`（本帧 + 下一帧各清一次并重新断言侧栏落点配置：dockview 会在落位后的一帧里重建覆盖层 / 重置 group 落点配置），`handleSidePanelMoved` 对所有面板先跑它、再做侧栏专属的列宽归一（侧栏行为不变）。另补两处兜底：① **落点被拒时面板根本不动**，`onDidMovePanel` / `onDidDrop` 都不会触发，因此在 `StudioView` 挂窗口级 `pointerup` / `pointercancel`（捕获）——下一帧若 `.studio-dock` 里还留着 `.dv-drop-target-container` 就补一次收尾；② `noteSidePanelWillStackDrop` 增 `draggedPanelId` 口径，只服务侧栏互拖：此前把对话拖到上下叠放的资产 / 参数上也会顺手武装列宽归一窗口并记下 `pendingStackColumnWidth`，可能在随后的 layout change 里误改侧栏列宽。验证：`typecheck` / `lint`（`check:cjk` 0 违规）/ `format:check` 通过，全量 275 文件 2061 项单测通过（新增 `tests/dockDropArtifactCleanup.test.ts` 3 项：普通页签拖到侧栏只清落点、不做列宽归一也不做防御性激活；dockview 下一帧重建的覆盖层同样被清；普通页签不武装归一窗口而侧栏互拖仍归一）
...
- **新增 MCP 工具 `render_svg`：让 Agent 自己在应用里「看一眼矢量画面」，不必去起浏览器**：`svg.gen` / `svg.anim` / `asset_import` 让工程里能存、能烘焙 SVG，但 Agent 看不见画面——沙箱里起不了本机浏览器（见下两条），它只能把 SVG 当纯文本读，「生成的矢量图到底长什么样」全靠猜，或者绕过沙箱去试浏览器（用户就挨一次 0x80000003 系统框）。本轮补上这条缺失的观察通道：新增只读工具 `render_svg`，入参 `svg`（内联标记）或 `svgPath`（工程内相对路径）二选一，另有 `frames`（2~60，默认 4）、`durationSec`（0 = 自动探测 SVG 自身动画周期）、`width` / `height`（上限 1024，缺省跟随 SVG 自身尺寸）、`background`（透明 / 白 / 黑），画面以 MCP image content 随响应回给客户端（与 `timeline_preview` 同一形态：多模态客户端直接看得见，纯文本客户端只读文本结果、不会报错）。**不新增任何渲染能力**：作业 `svg-raster` 走既有的渲染作业通道（主进程 `runRenderJob` → 界面进程执行，`MCP_RENDER_JOB_KINDS` 增一项），渲染层处理器复用图执行那条烘焙链 `renderSvgFrames`（解析 → 探测动画周期 → 逐帧求值并静态化 → canvas 栅格化）——静态 SVG 只出 1 帧、带动画按时间轴逐帧烘焙，与 `svg.anim` 出图口径完全一致，不存在「预览与产物两套算法」；`RENDER_JOB_HANDLERS` 是 `Record<McpRenderJobKind, …>`，漏写实现直接编译失败。**只读、不落盘**：只回 PNG 帧，不写资产库、不改工程（要落库走 `svg.gen` → `svg.anim`），因此登记进 `mcpModeAccess` 的只读集合，Plan 阶段规划前也能先看画面。帧数上限（`SVG_RASTER_MAX_IMAGES = 4`）与默认取样帧数（`SVG_RASTER_DEFAULT_FRAMES = 4`）写在共享层 `@shared/graph/svgAnim`：多模态通道按体积计价，1024 边长一帧就有数百 KB，60 帧能把一次工具响应撑到几十 MB，超出的帧只在文本结果里报数量与时长；离屏栅格化的边界（`<image href>` / Web Font / 外部 CSS 不加载）与「只回了前 N 帧」都写进结果的 `notes`。工具描述同时点明「沙箱里禁止起本机浏览器、视觉自查用这个」；GraphSkill `svg.motion` 的中英「用法」段把视觉自查由「看 svg.anim 烘出的帧」改指 `render_svg`（模型做矢量任务时必读的那份 SKILL.md 是最靠近现场的落点），与 persona 的 headless-browser 段、运行体守卫四处同口径。验证：`typecheck` / `lint` / `check:cjk` / `format:check` 通过，全量 274 文件 2058 项单测通过（本工具体没有新增单测：判定与栅格化都在界面进程里跑，node 环境测不到，栅格化本身由 `svgTimeline` 的 19 项既有单测兜底）

- **对话过程中反复弹出「msedge.exe 应用程序错误 unknown software exception (0x80000003)」（Agent 自己起无头浏览器做视觉自查，被沙箱挡住后崩在应用之外）**：用户截图报「AI 对话运行过程中经常出现这个错误」。真凶不是应用自身：Agent 为了对 SVG / HTML 产物做视觉自查，用 `pwsh` 起本机 `msedge.exe --headless=new --screenshot=…` 把矢量图 / 页面栅格化成 PNG，而 `pwsh` 跑在 dsh 的 Windows ACL 沙箱里（受限令牌 + `CreateProcessAsUserW`）——Chromium 建不出自己的 mojo IPC 命名管道（工具结果原文 `FATAL:mojo\public\cpp\platform\platform_channel.cc:183] Check failed: . : 拒绝访问。 (0x5)`，同一条 stderr 里还有 `crashpad_client_win.cc:447 OpenProcess: 拒绝访问`），release 版 `CHECK` 失败走 `IMMEDIATE_CRASH()` → `__debugbreak()` → `STATUS_BREAKPOINT = 0x80000003`，未处理异常由系统兜底弹出模态「应用程序错误」框（日志里的 `exit=-2147483645` 正是该码的有符号表示），而且工具调用照样失败、`--no-sandbox` 也救不了（失败点在 mojo 管道，不在 Chromium 自己的沙箱开关）。会话日志实测：近期 5 个会话出现 91 次 `--headless`、2 个会话 4 次该 FATAL，模型自己也诊断出了原因（`approval/asked` 原文：`Headless Edge needs to create its internal IPC named pipe … the confined sandbox blocks that pipe`）并要求升到 `danger-full-access`，但面板没有审批通道、`approval/decided` 记为 `unavailable`，于是它只能一遍遍在沙箱里重试、每试一次弹一个框。**沙箱侧拦不住这条路**：dsh 的沙箱策略只有文件效果（`dsh-sandbox-policy` 明确「network and process policy are outside its vocabulary」），没有命令黑名单可配；`dsh-pwsh-sandbox` 也只把 runner 的拒绝当事实回报。因此本轮从模型可读的提示词侧收口（**不改任何运行时链路**）：① persona 公共段新增 `=== Headless browser is unavailable ===` 一段（Ask / Plan / Craft 三种模式都带）：禁止从 shell 启动本机浏览器（`msedge.exe` / `chrome.exe`，含任何 `--headless` / `--remote-debugging-port` 调用）并写明后果（受限令牌下以 0x80000003 中止、在应用之外给用户弹系统错误框），禁止用 `--no-sandbox` 或其它开关重试、禁止为跑浏览器去申请沙箱提权，要光栅化或视觉自查就走 `svg.gen` → `svg.anim` 烘焙出的 PNG 帧与 GIF（`task_status` 会给出产物路径），真需要真浏览器时停下来让用户在应用外跑；② GraphSkill `svg.motion` 的中英「用法」段补同口径一句——矢量任务里模型一定会读到这份 SKILL.md，这是最靠近现场的落点（技能文件按签名变化自动重写落盘）。验证：`typecheck` / `lint` / `check:cjk` / `format:check` 通过，全量单测通过。**已知边界（留观）**：提示词是软约束，要硬保证只能在沙箱 spawn 补丁里对命令行做 Chromium 拦截（`@deepseek-ai/dsh-win32-process` / `dsh-sandbox-windows-acl`，与本仓库既有的「隐藏控制台窗口」补丁同一位置），代价是只能匹配命令文本、易误伤（`taskkill /IM msedge.exe` 这类清理也算命中）、动态拼路径的启动脚本仍拦不住——先看模型是否还去碰浏览器，再决定要不要上这层

- **上一轮「提示词禁止在沙箱里起浏览器」被模型绕过（用户再次看到同一个 msedge 0x80000003 系统框），改在进程创建处硬拦截**：上一条修复只加了 persona 与技能文本的软约束，用户随即反馈「又开始弹这个了」。解压会话日志后拿到实测形态：模型不再直接写 `msedge --headless=…`，而是把可执行文件路径存进变量再用调用运算符启动——`$edge='C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe'`、`& $edge --version`、`& $edge --headless=new --disable-gpu --hide-scrollbars --window-size=900,711 --screenshot="…" "…svg"`。这同时说明两件事：任何「按命令名遮蔽」的拦法（PowerShell 函数 / 别名、PATH 放同名 shim）对 `& $path` 无效——带路径的调用运算符不走命令解析；运行时也确无可用的收口点（`dsh-sandbox-policy` 只有文件效果策略，`dsh-pwsh-sandbox` 只把 runner 的拒绝当事实回报，真正启动 Chromium 的是 pwsh 自己 `CreateProcess`）。因此本轮落在**进程创建处**：Windows 上 `dsh-base` 只启用 pwsh（`bash-sandbox` / `tool-bash` 是 `disabled: process.platform === 'win32'`），而 pwsh 的两条链路——直连（`run` / `start`）与沙箱（`SandboxPwshExecutor.confine()` 把真实 argv 重包在 `--` 之后）——以及终端都最终走 `@deepseek-ai/dsh-subprocess-local` 的 `LocalSubprocessRuntime.spawn(spec)` / `spawnTerminal(spec)`，**一处插入即覆盖全部执行器、且不依赖任何执行器包的内部结构**（上一版收口在 `PwshLocalExecutor.argv(spec)` 只盖住 pwsh 的两条链路，作废）：新增运行体补丁 `dshBrowserGuardPatch`（沿用 ACL 控制台补丁那套「运行时写 `node_modules` + `bundle-dsh` 构建期重写」机制，两处各自幂等、找不到锚点即静默跳过），在方法体开头插入守卫——命中「启动浏览器」就不创建进程，直接抛出可读原因回给模型（写明受限令牌下必然 0x80000003 且弹系统框、换写法也没用、要看画面就调 `render_svg`、真需要浏览器请让用户在应用外跑），persona 那段同步补一句「这类命令会在运行前被拒」，让模型第一次就放弃而不是换写法试第二轮。判定按 **argv 形状**分两条，既一处收口又不误伤：① 程序位（argv[0]，以及每个 `--` 之后那一项——ACL 沙箱把真实 argv 包在 runner 之后）直接就是浏览器可执行文件 → 拦，零误伤（`taskkill /IM msedge.exe` 的 msedge 不在程序位）；② 程序位是 shell / 命令运行器时才把整条命令行当文本判定：Chromium 自动化开关（`--headless` / `--screenshot` / `--dump-dom` / `--print-to-pdf` / `--remote-debugging-port`）出现即拦；浏览器可执行名与自动化入口（`msedge` / `chrome` / `chromium` / `headless_shell` / `chrome-headless-shell` / `playwright` / `puppeteer` / `chromedriver`，裸名或全路径）命中后，只有落在「检查 / 清理」语境（`taskkill` / `Stop-Process` / `Get-Process` / `Get-Content` / `Remove-Item` / `Test-Path` / `where` 等动词之后 80 字符内、且中间没有 `;` `|` `&` 分隔）才放行。这样 `taskkill /IM msedge.exe` 这类清理不受影响，`rg -e msedge.exe` / `rg -- --headless` 这类把浏览器名或自动化开关当**搜索词**的命令也不会误伤（程序位是 rg，不是命令行）。补丁失败只在启动日志里记一行（写盘失败、上游换实现都算「回到老样子」），绝不影响对话。验证：新增 `tests/dshBrowserGuardPatch.test.ts` 36 项（真实越界命令与「检查 / 清理」语境的判定表、搜索词不误伤、`--` 之后的程序位、终端 `spawnTerminal` 同拦截、注入片段与 TS 判定函数逐例同结果、源码改写的往返还原与 CRLF 保持、幂等、上游换实现无锚点时原样返回、依赖树落盘、真实产物改写后 `node --check` 仍是合法 ESM、构建脚本抽出的片段与运行时常量逐字节一致），`typecheck` / `lint` / `check:cjk` / `format:check` 通过，全量 274 文件 2058 项单测通过。**已知边界**：仍是文本判定——把浏览器名拆开拼接、复制成别的文件名、或用自动化工具自带的私有 Chromium 副本（命令文本里不出现可执行文件名）仍可能绕到「启动 → 崩 → 弹框」；若真发生，下一层防线是在 dsh 预载 hook 里调 `SetErrorMode(SEM_FAILCRITICALERRORS | SEM_NOGPFAULTERRORBOX)`（`koffi` 在运行体里可用），让崩溃进程的未处理异常不再弹系统模态框

- **修复「提示词里写了参考、生成时却没有参考图」（`styleImages` / `characterRefs` 等参考图参数被当成未声明参数丢掉，写错的条目还会留下「有参考」的假象）**：这四个键（`styleImages` / `styleImagesUseGlobal` / `styleReferenceSubject` / `characterRefs`）由生成节点直接从 `node.params` 读取，却没有任何节点类型在 `defaultParams` 里声明它们，于是 `workflow_commit` 物化时按「未声明参数」整键丢弃——计划里写好的风格图与角色引用一个都到不了执行侧（产物没参考图，任务照旧报成功）；MCP `graph_edit` 的 `params` 虽是浅合并、写得进去，但值本身也可能是不可解析的：角色引用只写名字（执行侧的名称回退目录为空）解析不出参考图，风格图条目既没有 `libraryId`、也没有 `data:` 开头的 `dataUrl` 时，运行期要么抛 `STYLE_IMAGES_LOAD_FAILED`、要么静默丢掉工程全局风格（`styleImagesUseGlobal: false` + 本地风格图为空）。现两条写入路径共用一套校验（新增共享纯函数层 `src/shared/graph/referenceParams.ts`，`materializeGraphPlan` 与 `applyGraphEditOps` 各调一次）：`ALLOWED_PARAM_KEYS` 登记这四个键（放行只代表键合法，值是否可用另判），值交给 `sanitizeReferenceParams`——风格图走 `normalizeProjectStyleImages`（只认 `libraryId` 或 `data:` 的 `dataUrl`）、角色引用只留带 `imageUrl` 的条目并截到 `DEFAULT_MAX_INPUT_REFERENCES`、`styleReferenceSubject` 只收 `default` / `ui`、本地风格图为空时连开关一起丢掉并回落全局风格；丢弃的条目与被纠正的开关一律列进 `warnings`，不再静默假成功。`graph_edit` 侧按**合并后**的值校验（单独改 `styleImagesUseGlobal` 时看得见节点已有的 `styleImages`，不会把「本地风格图 + 关掉全局」的合法组合误判成无风格），且本次改动无法生效时保留节点原值——一次写错不抹掉检查器里配好的参考图配置；工具 `params` 描述同步点明各参数的合法形状，减少 Agent 靠猜键空跑一轮。验证：`typecheck` / `lint` / `check:cjk` / `format:check` 通过，全量 272 文件 2013 项单测通过（新增 7 项：物化 4 项、`graph_edit` 3 项）

- **修复全仓格式化把 Vue 模板里的多语句内联处理器拆坏、整份 SFC 编译失败**：`.prettierrc.json` 为 `semi: false`，prettier 会把指令值按 JS 程序打印并删掉语句分隔分号（`@click="a(); b()"` 变成两行 `a()` / `b()`），`@vue/compiler-sfc` 随即报 `Unexpected token, expected ","`；该坏形态平时被 dev server 的增量缓存掩盖，只在 tsconfig 变更触发全量重编译时才暴露（CI 的 typecheck 与单测都不编译模板，因此没拦住）。不能靠「补回分号」解决——`prettier --write` 会再次吞掉分号、原样复现，故收敛为单表达式：`NodeGraphEditor.vue` 右键菜单 14 处改为 `runCtxAction(() => …)`（由该 helper 统一执行动作并收起菜单，`pasteClipboardNodes` 的 Promise 由内部 `void action()` 吞掉），`WorldTable.vue` 的 kind 页签赋值收敛为 `selectKind(kind)`。用 `@vue/compiler-sfc` 全仓编译模板校验：修前 2 个文件报错、修后 0 错误，且 `prettier --write` 后复验不再退化
...
- **新增「一句话生成 SVG 矢量动画」剪映教程脚本（`website/tutorial-script/svg-motion-tutorial-script.md`）**：本轮把 SVG 矢量链路（`svg.gen` → `svg.anim`）补进技能目录与提示词后，「在对话里说一句『生成一个鹈鹕骑自行车的 SVG 动画』」成了可复现的完整流程，本脚本把它写成一条能直接拍的教程：11 个 AI 生成镜头（画面提示词 / 配音文案 / 字幕要点，约 110 秒）+ 6 条录屏镜头（对话面板切 Craft 说一句话、Agent 自动建图与连线、`.svg` 资产落库并放大看边缘、烘焙 GIF 与检查器四参数、导出 GIF 与逐帧 PNG，另附「图库 SVG 资产直接接烘焙」一条）+ 拼接与配音顺序 + 发布标题与简介。末尾另有一节**录屏前必看的避坑清单**：逐帧烘焙求值的是 SMIL 的 `animate` / `animateTransform` / `set`（`renderSvgFrames` 的 `ANIMATION_SELECTOR` 只收这三者与 `animateMotion`），于是写在 `<style>` 里的 CSS `@keyframes` 不参与求值、会被判为静态 SVG（只出 1 帧、不产 GIF），位移也不能用不参与求值的 `animateMotion`；再点明「要做矢量就别让它改走 `generate_image`」「接的是 `svg` 口，`image → svg` 单向兼容」等 7 条，避免教程录屏时踩空。纯文档新增，不动任何代码
