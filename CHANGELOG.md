...
节点图新增「3D 模型拆分」加工节点（`model.segment`），把上游 GLB 交给 Tripo 云端拆件 API 拆成部件：**网格分割** `POST /v3/mesh/segment`（几何拓扑；给了粒度或显式连通域开关时升级到 `v2.0-20260430` 语义 + 几何）与**智能分割** `POST /v3/mesh/smartsegment`（`seg_type=model` + 列主序 4×4 单位矩阵 + `granularity` coarse/medium/fine + 可选 `hint` 点名部件），两者都走异步任务：提交拿 `task_id` → `GET /v3/tasks/{task_id}` 轮询 → 下载拆分后 GLB。新增 `src/main/services/modelProviders/model3dSegment.ts`（提交 / 轮询 / 智能分割附加产物 mask + 部件描述）、`glbParts.ts`（**部件名不在 API 响应里**，按官方 ComfyUI 节点同款做法解析 GLB 的 JSON chunk → `nodes[].name`）、`shared/supportsModel3dSegment` + `SEGMENT_MODEL3D_PROVIDER_KINDS`（只放 Tripo）与 IPC `gen:model3d-segment`、`facade.segmentModel3d`（本地模型先上传对象存储换公网 URL）、`preload.studio.segmentModel3d`。图侧：`shared/graph/execute/modelSegment.ts` 把 `segmentMode/segmentGranularity/segmentSplitByConnectivity/segmentSmartGranularity/segmentHint` 映射成请求、把 `segmentParts/segmentDescription/segmentMaskUrl/segmentModelRelativePath` 写回节点参数，`ctx.segmentModel3d` 在图编辑器与后台任务两条派发路径都注入；卡片面板新增拆分模式与两套粒度下拉（网格 `simple/balanced/detailed` 与智能 `coarse/medium/fine` 枚举不通用），供应商下拉按 `supportsModel3dSegment` 过滤；新增 `ModelSegmentInspector.vue` 展示部件列表 / Tripo 部件描述 / mask 图；`graphGalleryOutput` 支持切换历史结果时同步预览路径。智能分割只接受 `.glb` 来源（quad/FBX 需先转 GLTF），指令框即「点名要拆的部件」提示词。单测覆盖两条端点的请求体与轮询字段、GLB 部件名解析（含去重 / 非 GLB 载荷）、execute 的参数映射与脏参数清理。

节点图再补四个 Tripo 后处理加工节点：**3D 部件补全**（`model.meshComplete` → `POST /v3/mesh/complete`，`completion_mode` = AI 补全 50 积分 / 快速封口 30 积分，`part_names` 可选）、**3D 重拓扑**（`model.retopology` → `POST /v3/mesh/decimate`，智能 v2.0 30 积分 / 基础减面 v1.0 10 积分，`face_limit` / `quad` / `bake` / `part_names`，v1.0 不支持后两者）、**3D 绑骨检查**（`model.rigCheck` → `POST /v3/animations/rig-check`，免费，返回 `riggable` + 推荐 `rig_type` 并把模型原样透传）、**3D 动画重定向**（`model.retarget` → `POST /v3/animations/retarget`，10 积分/动画，`animation` 与 `animations` 二选一，`out_format` / `bake_animation` / `export_with_geometry` / `animate_in_place`）；自动绑骨 `POST /v3/animations/rig` 已由既有「3D 骨骼蒙皮」节点覆盖（`rig_type` 映射 + `spec: mixamo` + `out_format: glb`）。关键约束：**部件补全只吃 `mesh/segment` 的任务 id、动画重定向只吃 rig 任务 id**，所以新增 `GraphAssetValue.providerTaskId`（贯通 `GraphModelItem` / `persistModelGeneration` / `dualModelGalleryOutputs`）让拆分与蒙皮节点把 Tripo task id 随输出值传下去，智能分割回填其中的 `seg_task_id` 子任务 id，卡片/Inspector 缺前置时直接提示「请接入拆件 / 蒙皮节点」。实现集中在 `model3dPostProcess.ts`（四条端点的提交 / 轮询 / token 前缀 + 绑骨检查的 submit/poll）、`facade.postProcessModel3d`（task id 优先、否则本地模型上传对象存储换公网 URL；绑骨检查无产物，就地 2s 轮询最长 180s，不走 videoJobService）、IPC `gen:model3d-post` + `preload.studio.postProcessModel3d`，四条图节点共用 `ctx.postProcessModel3d` 通道与 `ModelPostProcessInspector.vue`（补全模式 / 算法档位 / 目标面数 / 四边面 / 烘焙 / 输出格式 / 三个开关 + 绑骨检查结论卡片）；卡片指令框分别承载「要处理的部件名」与「预设动画 id」，供应商下拉只保留 Tripo。单测覆盖四条端点的请求体（含 v1.0 丢弃 `bake` / `part_names`、重定向单条与批量、绑骨检查 GLB 限制与 task_id 互斥）、轮询字段与 token 解析、四个 execute 的参数映射与任务 id 透传。

「3D 骨骼蒙皮」节点补齐 Tripo 的两个专属选项：卡片面板新增**骨架命名**（`spec`：`mixamo` 兼容 Mixamo 动作库 / `tripo` 原生）与**输出格式**（`out_format`：`glb` / `fbx`）下拉，选 Meshy 时自动隐藏（Meshy 无这两个参数），非法值在适配器层回落到 `mixamo` / `glb`，默认行为与之前完全一致。配套把 3D 产物落盘名从写死的 `output.glb` 改成按下载直链后缀推断（`resolveModel3dDownloadName`，覆盖 glb/gltf/fbx/obj/stl/usdz，拿不到后缀回落 glb）——否则 `out_format: fbx` 回来的 FBX 会被存成假 GLB、资产库与预览按 GLB 解析失败；这条同时给后续「格式转换」节点铺好路。参数经 `RigModel3dInput.spec/outFormat` → `submitCloudModel3dRig` 透传，节点侧写进 `generateRigSpec` / `generateRigOutFormat` 并随 Cook 回填。单测覆盖 spec/out_format 透传与非法值回落、execute 的参数映射与默认口径、下载名推断（含 query / hash / 未知后缀）。

再补两个 Tripo 模型处理节点，凑齐模型后处理全家桶：**3D 格式转换**（`model.convert` → `POST /v3/models/convert`，基础 5 积分 / 传任一高级参数即 10 积分，覆盖 `format` GLTF·FBX·USDZ·OBJ·STL·3MF + `quad`（强制 FBX）+ `force_symmetry` + `face_limit` + `flatten_bottom(_threshold)` + `texture_size` / `texture_format` + `bake` + `pack_uv` + `export_vertex_colors` + `pivot_to_center_bottom` + `scale_factor` + `with_animation` + `fbx_preset`（blender / 3dsmax / mixamo / bake_scale）+ `export_orientation` + `part_names` 子集导出）与 **3D 贴图**（`model.texture` → `POST /v3/models/texture`，重绘 / 文生贴图，`model` 贴图版本 v3.5-20260815 / v3.0-20250812 / v2.5-20250123、`texture_prompt` 文生提示词、`pbr`、`texture_seed`、`texture_alignment`、`texture_quality`（fast / standard / detailed / extreme 8K）、`part_names`、`delight`）。两者复用既有 `postProcessModel3d` 通道（token 前缀 `tripo-convert::` / `tripo-texture::`，任务路径表按 op 分发），`texture_quality=fast` 在非 v3.5 版本上提前拦下并给出明确文案（上游会直接 1004），`format` 缺失或非法同样前置报错而非透传 400。Inspector 为两节点各加一套表单（格式 / FBX 预设 / 面数 / 贴图尺寸与格式 / 四边面 / pivot / UV / 烘焙 / 保留动画 / 部件子集；贴图侧为版本 / 精度 / 对齐 / 种子 / PBR / 部件），卡片指令框分别承载「导出部件名」与「贴图提示词」，供应商下拉同样只留 Tripo。另核对过 `/v3/models/refine`（存在但无公开文档页，且我们生成侧不出草稿模型）与 `/v3/models/import`（其 task_id 对 complete / retarget 无用、URL 链路已覆盖），暂不做节点。单测覆盖 convert 的完整请求体、缺 format 报错与非法枚举丢弃、texture 的提示词/精度/对齐/种子/部件与 fast 版本校验。

修掉新增的「3D 模型拆分」与模型后处理 inspector **内容贴边**：inspector 的容器样式（`.node-inspector` 的 `padding: 12px` / `gap: 12px` / `height: 100%` / `overflow: auto`）在项目里不是全局定义的，而是每个 inspector 组件各自声明——`ModelSegmentInspector.vue` / `ModelPostProcessInspector.vue` 漏了这段，于是 head、提示、部件列表、参数表单全部顶到面板边缘。现按 rigSkin 同口径补齐容器、`.head`、`.hint`、`.section-hint`（虚线空态卡）、`.preview-slot`（`flex: 0 0 auto` + `width: 100%` + `:deep(.model-preview)` 撑满）与 `h4` 样式，表单标签加 `flex: 1 1 auto; min-width: 0` 防止长标签把控件挤出边缘。新增 `tests/inspectorContainerStyle.test.ts` 把「渲染了 `class="node-inspector"` 就必须自带容器内边距 + 行距」做成不变量（扫描 `inspector/builtins.ts` 注册的全部组件，当前 40 个全部合规；行距允许 10/12 两种既有口径），并要求两个模型加工 inspector 额外满足 `height: 100%` / `overflow: auto` / `.node-inspector.empty`。

3D 网格加工按「**能力矩阵（shared）/ 供应商方言（main）/ 编排层（与供应商无关）**」三层重构（第 1 步，零行为变化）：新增 `src/shared/meshOps.ts` 的 `MESH_OPS_CAPS` 作为「谁能做哪些 op」的唯一真相（`MeshOp` 八项 + `inputKinds` / `partNames` / `retargetIds` / `convertAdvanced` 能力位），`MODEL3D_RIG_PROVIDER_KINDS` / `MODEL3D_SEGMENT_PROVIDER_KINDS` / `MODEL3D_POST_PROCESS_PROVIDER_KINDS` 三个白名单改为由它派生，`GraphNodeCard.vue` 里那份硬编码 `tripoOnly` Set 也换成 `meshOpSupported(kind, op)`（8 个 3D 加工节点显式映射到 op），门禁从 5 处收敛到 1 处。新增 `MeshOpsDialect` 接口（`types.ts`，含 `createClient` / `buildRigRequest` / `buildSegmentRequest` / `buildPostProcessRequest` / `parseTask`）挂在 `ModelProviderAdapter.meshOps` 可选成员上，并按既有 adapter 注册机制落到 `tripo/meshOps.ts`（416 行）：Tripo 的端点路径、请求体构造、枚举与模型版本常量、状态字映射、base URL 归一化、响应解析与协议级前置校验（缺 task id / 缺 format / fast 版本 / GLB-only）全部集中于此；`model3dSegment.ts` / `model3dPostProcess.ts` / `model3dRig.ts` 只剩提交、轮询、错误归一与 token 编解码（编排层已无任何 `/v3/` 端点字样，`model3dRig.ts` 的 Meshy 内联分支留待第 2 步搬迁）。新增 `tests/meshOpsCaps.test.ts` 守住分层不变量：三个白名单必须等于矩阵派生值且锁定当前实际值（`['meshy','tripo']` / `['tripo']` / `['tripo']`）、`supports*` 与 `meshOpSupported` 口径一致、矩阵里声明了非 rig op 的供应商必须暴露方言、方言的端点与解析行为（含 extras / riggable / 失败原因 / 进度）。既有 54 个网格加工单测**一行未改**全部通过。

3D 网格加工重构第 2 步：**Meshy 方言落地 + token 去品牌化（含在途任务兼容）**。（1）新增 `meshy/meshOps.ts`，把原先内联在 `model3dRig.ts` 里的 `/openapi/v1/rigging` 协议（`result` 字符串/对象取 task id、大写状态枚举、`rigged_character_glb_url` / `basic_animations.walking_glb_url` 产物字段）搬进方言，并顺带修掉 `EXPIRED` 落进 pending 会一直轮询到瞬时失败上限的问题（按失败处理）；`MeshOpsDialect` 补两个跨家差异点——`parseSubmit(payload)`（Tripo `data.task_id` vs Meshy `result`）与 `pollPath(op, taskId)`（Tripo 统一 `/v3/tasks/{id}` vs Meshy 按任务族 `/openapi/v1/<任务名>/{id}`），新增 `meshOpsDialect.ts` 按 kind 取方言。至此 `model3dRig.ts` / `model3dSegment.ts` / `model3dPostProcess.ts` 三个编排模块**零供应商分支、零硬编码 kind 字面量**（纯提交 / 轮询 / 错误归一）。（2）新增 `meshOpsJob.ts` 统一 token 编解码：写侧改为 `<providerKind>-<jobOp>::<taskId>`（如 `tripo-smartSegment::x`、`meshy-rig::x`），读侧先试新格式再回退 9 条历史前缀（`tripo::` / `tripo-seg::` / `tripo-smartseg::` / `tripo-complete::` / `tripo-decimate::` / `tripo-retarget::` / `tripo-convert::` / `tripo-texture::` / `meshy::`），所以 `videoJobService.resumePending` 恢复旧版本落盘的在途任务仍能正确分发；`facade.pollModel3d` 的三段前缀判断收敛成一次 token 解析按 op 分发，`MODEL3D_*_TOKEN` 常量保留为历史前缀别名供兼容读取。`tests/meshOpsCaps.test.ts` 扩充到 12 例：新增 token 编解码（新格式写入、9 条历史前缀全部可解析、未知/空 token 返回 undefined）与「声明即实现」的方言契约（矩阵里每个 op 都要能构造出端点与轮询路径，含各自的必需入参）。全量 322 文件 / 2503 例通过。

3D 网格加工重构第 3 步：**Meshy 扩到贴图 / 重拓扑**（架构收益的第一次兑现——共享文件只改了能力矩阵与门禁口径，卡片下拉、IPC、图节点、任务落盘全自动跟上）。新增 `meshy/meshOps.ts` 的 `POST /openapi/v1/retexture`（`text_style_prompt` / `enable_pbr` / `remove_lighting` / `texture_resolution`，Tripo 的贴图精度按 fast·standard→2k、detailed→4k、extreme→8k 近似映射到 Meshy 的分辨率轴）与 `POST /openapi/v1/remesh`（`topology` quad·triangle、`target_polycount`、`origin_at`；Tripo 的智能/基础档 decimation_mode 语义不同故不透传），产物解析改成 op 无关的统一取值 `model_urls.glb` → `result.rigged_character_glb_url` → `model_url` → `model_urls.fbx/gltf`（retexture / remesh 的产物在顶层、rig 在 `result` 下）；`MESH_OPS_CAPS.meshy.ops` 扩到 `['rig','retopology','texture']`，`MODEL3D_POST_PROCESS_PROVIDER_KINDS` 相应变成 `['meshy','tripo']` 并加注释说明**不能拿它做门禁**（各家 op 覆盖不同）——facade 与卡片早已改为逐 op 的 `meshOpSupported(kind, op)`，这次把 facade 里最后一处按供应商整体放行的判断也换掉了。另外修掉一个跨供应商链路的真问题：上游带来的 `providerTaskId` 可能属于别家（Tripo 拆分节点后面接一个选 Meshy 的贴图节点），新增方言方法 `acceptsTaskId(taskId)`（Tripo 认 `task_*`、Meshy 认 UUID 形态）让编排层决定「直接用 id」还是「退回上传模型换公网 URL」，否则会把 Tripo 的 task id 当模型地址发给 Meshy。新增 `tests/meshyMeshOps.test.ts`（11 例：两条端点的请求体与映射、精度→分辨率换算、task id 命名空间判定、按任务族的轮询路径、retexture/remesh/rig 三种产物解析与 EXPIRED→failed），`tests/meshOpsCaps.test.ts` 增补各家 op 覆盖断言。全量 323 文件 / 2515 例通过。

3D 网格加工重构第 4 步：**Inspector 感知供应商，参数按能力位显隐**。新增渲染层 hook `useMeshOpsNodeProviderKind.ts`（与卡片同口径：节点参数里的 `generateProviderInstanceId` / `generateModel` 优先，未手动选过则回落到 `loadGenerateModelOptions('model3d')` 的默认选中项，取它的 `providerKind`）。能力矩阵新增 4 个 UI 能力位（`textureModelVersion` / `textureSeed` / `retopologyTier` / `retopologyBake`，加上已有的 `partNames`），Meshy 全为 false、Tripo 全为 true；`ModelPostProcessInspector.vue` 的贴图与重拓扑面板据此显隐 Tripo 专有控件——贴图的「贴图模型版本」「随机种子」「贴图部件」、重拓扑的「算法档位」「烘焙贴图」「处理部件」，Meshy 下这些控件不再出现。供应商未解析出来时一律按可用渲染（`caps?.x !== false`），避免面板先渲染一半再整片消失；非 Tripo 时面板顶部多一行说明（贴图：走 retexture、精度换算为 2k/4k/8k；重拓扑：走 remesh、按 topology + 目标面数），中英双语。新增 `tests/meshOpsInspectorGating.test.ts` 守住「能力位 → 控件显隐」的映射（5 个 flag 必须由对应能力位计算且用于 `v-if`、hook 必须走 loadGenerateModelOptions、说明文案两种语言都在），`tests/meshOpsCaps.test.ts` 增补 UI 能力位与两家实现一致性的断言。全量 325 文件 / 2519 例通过。

3D 网格加工重构第 5 步：**Meshy 动画重定向 + 动作库选择器**（至此 Meshy 覆盖 rig / retarget / retopology / texture 四项）。协议侧：`POST /openapi/v1/animations`（必填 `rig_task_id`，`action_id` 单个 / `action_ids` 批量，去重并丢弃非法值）、轮询 `GET /openapi/v1/animations/{id}`、产物解析补 `result.animation_glb_url` / `animation_fbx_url`；`GET /openapi/v1/animations/library` 免费全量返回，映射成统一的 `Model3dAnimationAction`（`action_id` / `name` / `category` / `sub_category` / `preview_url`，非数组安全降级为空）。能力矩阵 `meshy.ops` 加 `retarget`、`retargetIds` 由 `none` 改为 `library`（Tripo 仍是 `preset`）——于是「3D 动画重定向」节点的供应商下拉自动多出 Meshy。新增 `MeshOpsDialect.listAnimations?()` 可选方法与 IPC `gen:model3d-animations`（`facade.listModel3dAnimations` → 按 kind 取方言，未实现返回空数组），渲染层 `ModelPostProcessInspector.vue` 在 `retargetIds === 'library'` 时把「预设动画 id」输入换成**动作库选择器**：自动加载一次 + 搜索框 + 可点选的动作胶囊（选中态高亮、显示已选数量），选择结果落节点参数 `retargetActionIds`，Cook 时作为 `actionIds` 透传；Tripo 侧维持原来的预设字符串方式。跨供应商约束照旧显式拦下：Meshy 重定向只吃**它自己的** rig 任务 id（`task_*` 直接报「请把上游蒙皮节点切到 Meshy」），且必须至少选一个动作。新增 5 例 Meshy 方言测试（单个/批量 action、命名空间与空动作校验、产物解析、动作库映射与降级）、caps 测试补 `retarget` 覆盖与 `retargetIds: library`、执行层测试补 `actionIds` 透传、UI 门禁测试补动作库选择器的能力位约束。全量 325 文件 / 2525 例通过。

3D 网格加工重构第 6 步：**Meshy 格式转换 + 转换面板高级参数门禁 + Tripo 光照开关**。Meshy `convert` 接进来（`POST /openapi/v1/convert`，只有 `target_formats` 数组，必填；节点里的目标格式按 Tripo 口径映射成 Meshy 小写枚举，`GLTF → glb`；FBX 预设 / 面数 / 贴图尺寸与格式 / pivot / UV / 烘焙 / 保留动画 / 部件子集这些高级参数**一个都不发**，轮询走 `/openapi/v1/convert/{id}`），`meshy.ops` 加 `convert`。为此「3D 格式转换」面板按 `convertAdvanced` 能力位分成两段：通用段只留「目标格式」，高级段（Tripo）才显示其余 9 项控件，Meshy 下顶部给一行说明——这正是当初预留 `convertAdvanced` 的用途；部件子集继续走 `partNames` 能力位。另一处补漏：`delight`（去掉参考图烘焙光照）此前只在共享类型与方言里，UI 从未暴露；现补成三态下拉「默认 / 去掉 / 保留」，**未设置就不发**，从而保留各家默认（Tripo `delight` 默认开、Meshy `remove_lighting` 默认不动），避免我们替上游改默认行为。终态：Tripo 8/8 项、Meshy 5 项（rig / retarget / retopology / texture / convert）。新增 Meshy convert 测试（格式映射、高级参数不透传、task id 与 URL 两条输入、缺格式报错）与轮询路径用例，caps 测试同步；全量 325 文件 / 2526 例通过。

3D 网格加工第 7 步：**上游部件点选**（补齐「拆分 → 按部件操作」的最后一环）。此前拆出来的部件名只能手输进下游节点的指令框，而部件名等于拆分后 GLB 的 node 名、基本猜不出来；现在「部件补全 / 重拓扑 / 贴图 / 格式转换」四个节点的 Inspector 里新增 `MeshOpPartsPicker.vue`——沿 `in-model` 入边向上回溯最多 4 跳取第一个带 `segmentParts` 的上游节点（所以 segment → 部件补全 → 重拓扑 这类链式加工也能拿到同一份部件表），把部件渲染成可点选胶囊（选中态高亮 + 已选计数），点选结果写回各自的 `*PartNames` 参数。为避免「点完选择再动一下卡片指令框就被文本冲掉」，点选同时把结果按逗号回写 `generateInstruction`（与卡片 `[,，\n]` 的解析口径一致，双向可逆）；上游还没 Cook 过拆分节点时给明确空态提示。纯逻辑抽到 `features/graph/model/meshOpParts.ts`（`togglePartName` / `joinPartNames`，不可变、去重、trim）并配套单测，UI 门禁测试新增一条覆盖「四个节点都接了选择器 + 回溯路径 + 指令框同步」。全量 326 文件 / 2529 例通过。

修掉 3D 加工节点「指令预设按钮点了没反应」并补齐该有的预设。原因有两个：一是当时给 7 个新节点统一填了空预设数组（判断它们的指令框是「部件名 / 提示词 / 动画 id」而非创作指令），二是 `GraphInstructionMentionEditor` 的预设**按钮**只判 `presetKind`、预设**菜单**却要 `presets.length`，于是空预设的节点上留了一个点开什么都没有的按钮。现按「指令框到底吃不吃文本」分两类处理：**吃文本的三个节点补上成套预设**——3D 贴图 10 条材质模板（磨损皮革 / 拉丝金属 / 做旧木纹 / 陶瓷釉面 / 织物 / 卡通平涂 / 赛博朋克 / 石材 / 湿润表面 / 玻璃，写入 `texturePromptText`）、3D 模型拆分 6 条点名模板（游戏角色含武器护甲 / 机械载具 / 家具 / 建筑构件 / 卡通角色分肢 / 生物分肢，写入智能分割 `hint`）、3D 动画重定向 6 条动作组合（单个走路 / 待机+走+跑 / 移动+攻击四连 / 受击+倒地 / 转身+跳跃 / 舞蹈，写入 `animations`）；**指令框只写上游部件名或没有输入的四个节点**（部件补全 / 重拓扑 / 绑骨检查 / 格式转换）保持无预设，但预设按钮改为 `v-if="presetKind && presets.length"` 直接隐藏。另加一处供应商标注：动作库型供应商（Meshy）的动画重定向不吃 `preset:xxx`，卡片按能力位把预设面板关掉（`:preset-kind="effectivePresetKind"`），动画改在 Inspector 的动作库选择器里选，避免点了预设反而报「请至少选一个 action_id」。菜单标题补 `titleModelSegment` / `titleModelRetarget` / `titleModelTexture`（中英）。新增 `tests/meshOpInstructionPresets.test.ts` 守住四条不变量：吃文本的 kind 预设 ≥6 条且 body/titleKey 合法、部件类 kind 必须为空且按钮隐藏、**每个预设的 titleKey 在两种语言里都有文案**（含既有 rigSkin / modelPose，防拼写漂移）、动作库型重定向不给预设。全量 327 文件 / 2535 例通过。
...
右键菜单分组调整：把**图片反推提示词**（`image.toPrompt`）、**提示词优化**（`prompt.optimize`）与**备注**（`note.text`）三个节点并入「文本」分组（📝，现为 文本 / 选取文本 / 备注 / 提示词优化 / 图片反推提示词 五项），并删掉独立的「提示词」分组。此前这三者的归属是割裂的：「备注」不属于任何分组、只能落在根菜单里（根菜单那条注释还写着「备注、提示词优化等」），提示词类的两个节点又另占一个分组，而三者语义上都属于文本类。删除分组时四处一起清：`CONTEXT_MENU_RESOURCE_GROUPS` 里的组声明、`ResourceMenuGroupId` 联合类型里的 `'prompt'`、分组标签与图标三元分支里的 `group.id === 'prompt'`（图标 ✨ 一并去掉，文本组继续用 📝），以及两侧语言的 `graph.context.groups.prompt` 文案键——留着就会变成「items 为空被 filter 掉、但 id / 文案仍在」的空分组残留。新增 `tests/contextMenuTextGroup.test.ts` 按源码扫描守住三条不变量：文本分组的 typeId 必须正好是这五项、`prompt` 分组与相关引用不得复活（组声明 / id 联合类型 / `group.id === 'prompt'` 三处）、两种语言都不得留孤立的 `groups.prompt` 文案键。全量 327 文件 / 2538 例通过，typecheck / cjk 门禁 / prettier 干净。

「一句话 3D 游戏」改由 **AI 对话面板**驱动（第 1 步：资产层脚手架 + dsh 技能 + 三个 MCP 工具）。背景：v6.6 的可玩 HTML 只有图节点一个入口，提示词里 3D 专规只有一句「简单几何体即可」，模型于是堆 Box/Sphere，恰好放弃了程序化生成最值钱的部分；对话侧则完全没有工具可用（45 个 MCP 工具里没有任何 gameplay 入口）。这一步把「生成者」换成当前可见会话，宿主只负责落脚手架 / cook / 报状态：

- **资产层脚手架**（`main/services/gamePlayScaffold.ts` 重写）：新增 `src/core/{rng,palette,registry}.js` 与 `src/assets/{geometry,texture,material,audio,level}/`，样例入口从「一个旋转的 Box」升级为「注册表 + 5 个程序化资产」——种子随机（`hashSeed`/`createRng`/`rngHelpers`/`createGameSeed`）、自洽配色（`hsl`/`createPalette`）、`defineAsset({id,kind,build})` 注册与 `buildAsset` 取用、Canvas 2D 画贴图（返回 `HTMLCanvasElement`，2D/3D 通用）、WebAudio 合成音效（Oscillator + BiquadFilter + 包络 + 噪声脉冲，噪声用本地 LCG 保证可复现）、环形关卡布局数据。2D 工程自动剔掉只服务 three 的模块，`src/core/*` 一律不 import three（否则 2D 构建直接失败）。
- **dsh 技能 `gameplay.proc-assets`**（`shared/graph/graphSkills.ts`，新增 kind `gameplay`）：把资产层目录、`defineAsset` 契约、七条硬规则（禁止 `Math.random`、贴图必须 Canvas 画、音效必须合成、禁止自定义 shader / `eval` / 外链）与工具流程做成按需加载的技能，随 `syncDshSkills` 快照到 `$DSH_HOME/skills` 注入 `<available_skills>`；`GAME_PLAY_DSH_SKILL_HINT` 这个「定义了但全仓无人消费」的常量到此有了对应的技能。
- **三个 MCP 工具 + 作业服务**：`shared/gamePlayJob.ts`（契约：工程路径校验 `resolveGamePlayProjectRelativeDir` 只认 `<Cache/GamePlayJobs>/<jobId>/project`、模式归一、日志环形截断）与 `main/services/gamePlayJobService.ts`（prepare / build / status 状态机，构建后台跑、同工程复用同一条记录，所以对话里「再改一版」不会刷出一串作业）；`gameplay_prepare_project`(write) / `gameplay_build`(generate) / `gameplay_job_status`(read) 按 `mcpModeAccess` 分级——`gameplay_build` 不花模型额度但要跑 npm，按生成类收口。cook 复用既有 `buildGamePlayProject`（只加了一个可选的逐行 `onLog` 回调做实时日志），**npm 仍只由宿主跑**。
...
6.4.0 版本：在节点图里新增「3D 骨骼蒙皮」与「3D 动画」两类加工节点，参照 `model.pose` 的两条派发路径设计——**预设派发**本地查表确定性地走完（不烧 token）、**LLM 生成 Python** 多轮 agent 把 Blender 当成骨骼拓扑 / 关键帧重算的真相来源（避开 LLM 直吐欧拉 / 度数 JSON 的口径不稳）。`model.rigSkin` 节点接 3D 模型出 `rigMeta`（armature 名 + 骨头表 + 自动权重 vertex groups），内嵌 4 个预设拓扑（humanoid-simple 21 骨 / humanoid-mixamo 含手指+toe 直接挂 Mixamo 动作库 / quadruped 四足 / prop-rigid 单骨道具）；`model.animation` 节点接「rig 过的」模型出 `clip`（action 名 + fps + 帧范围 + 「骨名 → 帧号 → 局部欧拉弧度」关键帧表），内嵌 16 个游戏常用循环 / 静帧预设（idle / walk / run / jumpAir / jumpLand / wave / handsOnHips / think / crouch / kneel / bow / fightGuard / sit / hitReact / death / celebrate——前 13 个是通用循环 / 静帧，后 3 个为游戏特化的受击反馈 / 倒地 / 胜利庆祝），与 `model.pose` 共用弧度 XYZ 欧拉偏移约定。新建 `src/shared/blenderRigSkinGeneration.ts` / `blenderAnimationGeneration.ts` 提供策略选择、预设脚本构造（`buildBlenderPresetRigScript` / `buildBlenderPresetAnimScript`——按对应骨架 / 关键帧表生成完整 bpy 脚本，末两行 print `RIG_RESULT:` / `RIG_DRIVE_META:` 或 `ANIM_RESULT:` / `ANIM_DRIVE_META:`，stdout 解析器复用 AI 姿势同款 marker + 配对花括号 JSON + 类型守卫 + 平衡 JSON 扫描）、LLM prompt（强制「5 行 import 限制 + 末两行 print」）、stdout 解析、Node 端纯执行镜像（不连 Blender 也能算 readback 供单测与节点图 Cook 用）、多轮 `runBlenderRigAgent` / `runBlenderAnimAgent`（`try_blender_*` 工具 + `finalize_*` 工具，max 5 轮，agent 决策可多次返工脚本、靠 Blender 回传的 bones_matched / vertexGroupsMatched / keyframe_count 决定继续改还是收尾）。两套共享层都加完整单测（35 个新增 case：preset 派发 / 拓扑一致性 / Mixamo 命名 + parent 链闭合 / 脚本含 marker / stdout 解析成功-缺 marker-JSON 畸形-空数据 / 解析器 balance JSON / agent 单轮 OK 与首轮失败后次轮成功两条 happy path）。`GraphNodeTypeId` 联合加 `model.rigSkin` / `model.animation`；`GraphNodeParams` 加 `rigMeta` / `rigPresetId` / `clip`（复用既有 `animPresetId`）；`GraphAssetValue` 加 `rigMeta` 与 `clip` 字段（导演台 / 套用姿势侧按帧驱动时读这两个扩展）；执行器（`executeModelRigSkinNode` / `executeModelAnimationNode`）严格走 `model.pose` 同款 preset→readback→LLM agent 路径，`useGraphRunSession` 错误码 + i18n（中英）都按 `GRAPH_MODEL_RIG_*` / `GRAPH_MODEL_ANIM_*` 配齐，`graphPolicy` / `graphGenerateNodes` / `graphNodeTypeI18n` 三处白名单同步更新；右键菜单 `CONTEXT_MENU_RESOURCE_GROUPS.model3d` 一并把 `model.rigSkin` / `model.animation` 收入「3D」分组（与 `asset.model3d` / `asset.motion` / `model.pose` 同组，🧊 图标），不再混入根菜单的「未归入资源类型的节点」；`instructionPresets` 加 `modelRigSkin` / `modelAnimation` 两个 group（modelAnimation 同步灌入 16 个游戏常用动画预设，modelRigSkin 留待下版本补）；双击 `model.animation` 卡片打开指令面板，inspector（`ModelAnimationInspector.vue`）只保留运行 / 输出预览，动画指令输入框与 16 个常用动画预设都搬到卡片下方的「生成指令」面板，placeholder 文案与 hint 都按「游戏常用动画」定位改写。`model.rigSkin` 节点的 LLM agent 改用「**多轮 Blender MCP 调用直到成功**」的策略：`BLENDER_RIG_AGENT_DEFAULT_MAX_TURNS` 由早期硬编码 5 轮放宽到 50 轮（兜底防意外死循环），LLM 每一轮根据 Blender 回传的 `RIG_DRIVE_META`（bones_matched / vertexGroupsMatched）逐次调整 Python——参数、命名、parent 链、ARMATURE_AUTO 触发方式、mode 切换都是常见返工点，5 轮硬截会把大量合法拓扑拦在外面；同时加「**连续 3 次 Blender 回传相同 readback 指纹**（同 armature + 同 bones + 同 vertex groups）」的卡死检测，超阈值立刻终止并报 `stuck` reason，避免 agent 在原地微调同一份脚本反复烧 token；agent 系统 prompt 重写，明确「目标是 Blender 端真创建完整拓扑」「readback 不完整就不要 finalize」「连续无进展请重新审视拓扑而非微调」。新 result enum `stuck`（与既有 `finalized` / `no_finalize` / `max_turns` / `error` 并列），`BlenderRigAgentEvent` 加 `{ kind: 'stuck', identicalStreak }`。`tests/blenderRigSkinGeneration.test.ts` 加 3 条新 case——`BLENDER_RIG_AGENT_DEFAULT_MAX_TURNS >= 20` / `BLENDER_RIG_AGENT_STUCK_THRESHOLD === 3` / 模拟 LLM 第 7 轮才给到正确 Python 仍 finalize / 连续相同 readback 命中阈值后 stuck 退出 / 失败重试不污染 streak 计数。

`ModelRigSkinInspector.vue` 加**输入端 3D 模型预览**区块：rig 本身不重写模型文件，inspector 展示的就是上游资产节点生成的同一份 glTF/GLB/FBX/OBJ，让用户在跑 agent 之前/之后都能看到模型长什么样、对比 rig 后的拓扑差异。新增 `GraphNodeParams.rigModelRelativePath` 字段，`executeModelRigSkinNode` 在 Cook 完后用 `ctx.patchNode` 把 `model.relativePath` 写进节点 params（与 `rigMeta` 同一写入时机），inspector 直接读这个字段喂给 `ModelPreview` 组件；不要从入边回溯——多级 pipeline（rigSkin → rigSkin → ...）里回溯到第一个资产节点反而会拿到未 rig 的原始文件。新区块标题/空状态复用现有 i18n `graph.inspector.generate.modelPreview` / `modelPreviewEmpty`（ShotNodeInspector 同款键）。

`ModelRigSkinInspector.vue` 改造成**「模型 | 骨骼」双 Tab 布局**：上一轮新加的「模型预览」与「rigMeta 信息（armature + bones + vertex groups + presetId）」现在拆成两个互斥页签——Tab 标签走专属命名空间 `graph.inspector.modelRigSkin.tabs.{preview, skeleton}`，不复用 `asset.inspector.tabs.*`（后者是资产 inspector 用作「预览 / 动画 / 骨骼」三 Tab，文案含义不同）。默认 Tab = `'model'`，与上一轮加「模型预览」之前的打开效果一致，避免老用户突变感。「骨骼」Tab 数据源是 `params.rigMeta`——没有 rigMeta 时显示提示语 `skeletonEmpty`（运行节点后出现），避免空 Tab 看起来像 bug。i18n 新增 9 个键（zh-CN / en-US 各 9 个一一对应）：`tabsAria` / `tabs.preview` / `tabs.skeleton` / `armature` / `preset` / `bones` / `vertexGroups` / `skeletonEmpty`。复用 i18n 守卫（`tests/i18nLocales.test.ts`）强制两套文案 key 集合完全一致——直接在 modelRigSkin 命名空间下新建，不踩共享 `tabs.*` 防止日后再有 inspector 复用时串文案。

骨骼 Tab 加 3D 空间预览：仿 AssetInspector 三 Tab inspector 的共享 ModelPreview 模式——把 `ModelPreview` 提到 tabs 下、所有 panel 之上单一实例，根据 `activeTab` 切 props（`showSkeleton` 与 `selectedBone`），避免双实例双 WebGLRenderer 的性能与上下文开销。模型 Tab `showSkeleton=false` 看 mesh；骨骼 Tab `showSkeleton=true` mesh 隐藏、显示 Maya/Blender 风格的双锥骨架 joint + 父子连线（橙色），与 Perspective 一致：`selectedBone` 同时被 3D 视图（点击 joint / 连线触发 `select-bone` 事件）与列表（点击骨头名 toggle）双向更新；空选时全部显示，选中时 joint 变青色 + 配套 AxesHelper 显示局部坐标。骨骼列表数据源以 ModelPreview 从模型实际扫出来的 `THREE.Bone` 名为准（fallback 到 `rigMeta.bones`），避免 agent 上报的字符串与实际节点命名不一致导致错位点击。vertex groups 是蒙皮权重映射（顶点×骨骼矩阵），3D 空间无法直接可视化——保留为文字 compact 列表，让用户在跑 agent 之前/之后对比蒙皮拓扑。i18n 新增 2 个键：`skeletonHint`（骨骼 Tab 操作提示）、`bonesEmpty`（模型无 THREE.Bone 时的降级文案）。

修「骨骼预览为啥是空白」bug：上一轮说的"共享 ModelPreview"在实现上没做到——原本用了双 `<section v-show>` 各挂一个 ModelPreview，而 `v-show` 只切 `display:none` 不卸载组件，第二个 ModelPreview 实例的 viewport 在 `display:none` 下 `clientWidth/clientHeight=0`，WebGLRenderer 创建的 canvas 被 `Math.max(..., 1)` 兜底成 1×1 + ResizeObserver 永远不会触发，看上去就是空白。现在 ModelPreview 是 tabs 下、所有 section 之外**真正单例**：始终位于 layout flow，切 tab 只切 props（`showSkeleton` / `selectedBone`），不重建 WebGLRenderer / canvas / scene；panel 改用 `v-if` 控制（content 切到不挂载的 Tab 不会有性能成本，只是 hidden text），与 AssetInspector 三 Tab inspector 的复用模式完全对齐。

再修「骨骼 Tab 只剩网格、连模型都不见」——真正根因是**设计层缺口**：`executeModelRigSkinNode` 的 preset 路径只调 `computePresetRigReadback` 从拓扑表读骨头名生成 rigMeta 元数据，那份 Blender Python 脚本（`dispatch.code`）根本没被执行，输出模型就是原输入文件、里面一根 `THREE.Bone` 都没烘焙。骨骼 Tab `showSkeleton=true` 把全部 mesh 隐藏后场景里啥都不剩，只剩 GridHelper。修法是让预设骨架**可见**：ModelPreview 新增 `presetBones` prop（`BlenderBoneSpec[]`，armature 空间 head/tail 坐标），模型扫不出 `THREE.Bone` 时按预设拓扑**合成骨架预览**——骨架包围盒等比缩放对齐到模型包围盒，joint 球 + 双锥骨段与自带骨骼同一套材质/拾取/列表联动，点击选中、AxesHelper 全部可用；同时新增 `skeleton-source` 事件（`baked` / `preset` / `none`），inspector 据此切换提示文案（i18n 新增 `skeletonPresetHint`：「模型文件还没有烘焙骨骼，这里按预设拓扑合成显示骨架」）。注意预设骨架是拓扑预览而非蒙皮结果——骨骼真正写进模型仍需走 Blender MCP（AI 路径）。

「3D 骨骼蒙皮」节点（`model.rigSkin`）派发**统一收口到 Blender MCP**——之前 preset 路径走本地 `computePresetRigReadback` 短路，根本没把 `dispatch.code` 这份 Python 脚本发到 Blender 跑，导致预设路径下输出模型文件其实没被真加上 armature / 蒙皮权重、`rigMeta.vertexGroups` 也始终为空数组。修法：抽出 `runBlenderRigScript(ctx, presetId, code)` 助手直接调 `ctx.runBlenderMcpTool({ name: 'execute_blender_code', args: { code }, timeoutMs: 90000 })`，预设分支把 `dispatch.code` 经同一条 MCP 链路发出去，让 Blender `parent_set(type='ARMATURE_AUTO')` 真把 armature + 自动蒙皮权重烤进原文件，stdout `BLENDER_RIG_RESULT_MARKER` 解析回 readback（含真 vertex groups）；AI 分支保持原样走 `runBlenderRigAgent`（agent loop 内部同样经 MCP）。统一派发后两条分支契约一致：**先决条件 = `ctx.runBlenderMcpTool` 已注入**，缺则统一抛 `GRAPH_MODEL_RIG_MCP`（i18n `graph.run.modelRigMcp` 给出明确「请先启用 Blender MCP」文案）。`tests/modelRigSkinExecute.test.ts` 新增 4 个 case：预设路径必经 `runBlenderMcpTool`（断言 `name === 'execute_blender_code'` + code 长度 ≥ 200 + output 含 `vertexGroups` + `presetId === 'humanoid-simple'`）/ 未注入 MCP 抛 `GRAPH_MODEL_RIG_MCP` / `outcome.error` 透传失败原因 / 上游无模型 + 空指令两种前置错误。`computePresetRigReadback` / `executeBlenderPresetRigPure` 在共享层保留供单测与离线预览路径使用。

3D 姿势节点（`model.pose`）inspector 加「姿势预览」+「保存到资产库」按钮。**预览**：ModelPreview 新增 `bonePose` prop（骨名 → 局部欧拉**弧度**，与 `GraphNodeParams.bonePose` 同口径），加载完模型时抓每根 `THREE.Bone` 的 bind 局部 quaternion，套姿势时用 `bind.quaternion × eulerOffset.quaternion`（与导演台 `applyBonePoseForObject` 同口径——rad 单位，FK 子骨自然跟随父骨变形），watch prop 切换姿势时也走同条链路；ModelPoseInspector 把 `node.params.bonePose` 与 `node.params.poseModelRelativePath`（execute/modelPose.ts 新缓存的字段，与 rigSkin 的 `rigModelRelativePath` 对称）喂进去，没 Cook 结果降级为「请先连接模型」/「运行节点后查看」提示。**保存**：inspector 头部右上角「保存到资产库」按钮走 `SaveAssetDialog` 选文件夹，调 `window.studio.createAsset({ type: 'model', genParams: buildPoseAssetGenParams(bonePose, sourceAssetId) })`，与导演台 `saveObjectPoseAsAsset` 共用同一条姿势资产链路（`genParams = { modelKind: 'pose', pose: { schemaVersion: 1, bones, sourceModelAssetId } }`，导演台 `applyPoseAssetToObject` → `mapNormalizedPoseToTargetBones` 据此做骨骼名映射）。i18n 新增 10 个键（`posePreview` / `poseHint` / `noModel` / `poseEmpty` / `saveToAsset` / `saveDialogTitle` / `saveDialogSubtitle` / `saveDialogDefaultName` / `saveDone` / `saveFailed`）。按钮在 `bonePose` 为空时禁用，Cook 未运行前不会误触发。

3D 动画节点（`model.animation`）inspector 加「动画预览（时间轴 + 播放/暂停）」+「保存到资产库」按钮。**预览**：ModelPreview 的 `previewClip` 只能放 GLB 内嵌 `AnimationClip`，运行时生成的关键帧字典（`clip.keyframes` 是「骨名 → 帧号 → [x,y,z] 局部欧拉弧度」）走不通——所以复用上一次加的 `bonePose` 通路：当前帧 = 在关键帧之间按帧号线性插值得到的局部欧拉姿态，喂给 ModelPreview → bind × Euler 子骨 FK 跟随 / SkinnedMesh 按 vertex 权重自动 blend，与姿势预览同口径（rad 单位）。新增帧级 scrubber（拖动单帧定位）+ play/pause 按钮，rAF 按 clip fps 节拍推进 currentFrame、循环到起帧；clip 元信息（动作名 / fps / 总帧数 / preset）以 dl 表格展示。i18n 新增 16 个键（`animPreview` / `animHint` / `noModel` / `animEmpty` / `play` / `pause` / `frameLabel` / `clipName` / `fps` / `frameCount` / `preset` + 5 个保存键）。**保存**：domain.ts 新增 `AnimationAssetData` + `createAnimationAssetData` + `readAnimationAssetData` + `buildAnimationAssetGenParams`（与姿势资产对称），`genParams = { modelKind: 'animation', animation: { schemaVersion: 1, clipName, fps, frameRange, keyframes, presetId, sourceModelAssetId } }`；`isAnimationModelAsset` 早已识别 'animation' modelKind 这次把数据形状落稳。源模型路径与 assetId 来自新加的 `node.params.animationModelRelativePath` / `animationSourceAssetId`（execute/modelAnimation.ts 新缓存，对称于 pose / rigSkin）。stage `loadAnimationAsset` 仍按 GLB 文件加载，「缺文件则按 genParams 构造 AnimationClip」的集成留作跟进——这次先打通 inspector 预览 + 资产库入口。

人形绑骨不再让 LLM 猜 head/tail：`blender.rigSkin` brief 灌入确定性 `AIAE_HUMANOID_RIG` 脚本（`src/shared/blenderHumanoidLandmarks.ts` + `blenderHumanoidRig.ts`），按网格顶点 + 边连通 BFS 走到手脚与躯干摆 21 骨，再 `parent_set ARMATURE_AUTO`；端点走不通才回退固定比例。3D 节点 Inspector 补历史结果画廊（`generatedModels` / `selectedModelId`），与图/视频画廊同口径——选择、删除、保存到资产库；`asset.model3d` / `model.rigSkin` / `model.pose` / `model.animation` 共用 `persistModelGeneration`。侧栏 dock 卸载时清掉 layout 定时器，避免 headless `npm test` 里 `window is not defined`。

## [6.3.3] — 2026-09-17

6.3.3 版本：3D 导演台「AI 姿势生成」重接为 **Blender MCP 派发**，不再让 LLM 直接吐 Euler 度数 JSON。原 LLM 写数字角度本就拿手不稳（°/弧度混、关节链条不自洽、四元数 vs 欧拉混），新管道让 Blender 当姿势计算的真相来源——骨骼拓扑、链式约束、IK 解算、姿势库都是一手资产。两条路径共用同一套 `runBlenderMcpTool({name:'execute_blender_code'})` + stdout `POSE_RESULT:` 读回解析层：（1）**预设派发**——14 个 AI 姿势预设（idle / walk / run / jumpAir / jumpLand / wave / handsOnHips / point / think / crouch / kneel / bow / sit / fightGuard）各自对应一份手写 Python 脚本（按 `inferBoneRole` 算出的 `boneRoles` 查表输出 `pose_bones`），中英文 instruction 命中预设即跳过 LLM 调用——确定性、无 token 成本，弧度直接写回 `applyObjectBonePoseMap`；（2）**LLM 生成 Python**——自由文本描述走 `generateText`，prompt 强制 LLM 输出「import json + 构造 `POSE_BONES` dict + 末行 `print("POSE_RESULT:" + json.dumps(...))`」，再发到 Blender 跑。两端输出弧度（与 `StageVec3` 同口径，全程一处单位无来回转换），新建 `src/shared/blenderPoseGeneration.ts` 提供策略派发、预设脚本构造、LLM prompt、stdout 解析与 Node 端纯执行镜像（供单测在不上 Blender 时也能断言预设表与脚本一致性）。为打通 UI 调 Blender，**新开 IPC `MCP_BLENDER_RUN_TOOL`**（主进程白名单 `BLENDER_TOOLS` 内工具 + safe mode 硬约束），preload 暴露 `window.studio.runBlenderMcpTool`，渲染层不再走 MCP HTTP。Blender MCP 未连接时给具体开法指引（设置 → 工具面 → Blender / 启用 Blender MCP 插件），与 `runLogs` 增加 `kind: 'blenderMcp'` 日志条目（含 `toolName` / `codeBytes` / `strategy` / `presetId`）方便事后排查。
...
6.3.0 版本：把 dsh Agent 与 Blender / 第三方 stdio MCP server 接通——新增 stdio→HTTP 反向桥（`scripts/mcp-stdio-bridge.ts` 起 streamable-http 端点、行缓冲把 stdio 包成 HTTP，请求 / 通知流 / 健康检查都按 MCP 协议跑），并把 `mcpServerService.startMcpServer` / `closeMcpServer` 各自起 / 杀桥；`writeDshConfig` 在桥就绪时追加第二条 `mcp-client` 注册（`serverName: blender`、token 经 `STUDIO_BLENDER_MCP_TOKEN` 注入），dsh spawn env 多注入端口与 token；dsh 即能像直连一样消费 stdio MCP server。在此之上完整接入 Blender 工具集——直连社区 addon.py 的 TCP socket（`shared/blenderMcp.ts` 帧编解码 + `blenderMcpService.ts` 队列 / 超时 / 探活 / 错误归一）做一整套契约，并适配官方 Blender Lab「MCP Server」扩展（`shared/blenderOfficialTools.ts`：9 个工具各编成自包含 Python 段、执行帧 + `\0` 分隔），两种后端对模型侧完全同构；MCP 工具集 UI 改成「场景 / 对象 / 代码 / 导出 / 查询 / 资源」六 tab。Blender stdio 桥的启动命令 / 参数 / addon 主机端口 / safe mode / 桥 HTTP 端口一律从设置面板写（`BlenderMcpSettings` + `normalizeBlenderMcpSettings` 兜底），UI 可点「重启 Blender 桥」并实时看连接状态 / 版本 / 协议版本 / 实际可执行文件（`resolveExecutable` 跨平台 `where` / `which` 解析、找不到时红字提示「Windows spawn 不走 PATHEXT、建议填绝对路径」）；spawn 时透传 `BLENDER_HOST` / `BLENDER_PORT` / `BLENDER_MCP_SAFE_MODE`（默认 1、白名单阻断文件 I/O / subprocess / 网络 / 持久 hook），env 只保留 `PATH` + `ELECTRON_RUN_AS_NODE` + 7 个 AIAE_BLENDER_MCP_*，不再继承主进程杂项 env；进程死 / 未起 → 请求回 -32603「blender-mcp 未运行」。MCP asset_import 改为先落 `Cache/imports/` 中转（不再写 `Assets/`、不再广播 `ASSET_UPDATED`、不排队视觉打标 / 视频打点），对话产物卡自动命中「保存到资产库」由用户决定是否真正入库；UI 资产库的「导入素材」按钮仍走原 `importAssets` 路径，两条链路语义分离。3D 节点接入上游骨骼蒙皮（rig）——`GenerateModel3dInput` / `GraphNodeParams` 加 `rig` / `rigType` / `rigAnimation` 三字段，tripo 发 `body.rig`、meshy 发 `rigging` + `rig_type` + `pose`、hyper3d multipart 发 `rig` + `rig_type` + `rig_animation`，luma / lux3d 入口守卫拒绝传 rig；新增 `Model3dRigControls.vue`，节点卡片仅在 `providerKind ∈ {tripo, meshy, hyper3d}` 时挂控件。设置面板新增「联网搜索」顶层 tab 与 `SearchProvidersPanel`——内置 tavily / brave / serpapi / deepseekSearch + mock 5 个 adapter（共用 `http.ts` / `catalog.ts` / `types.ts`，通过 `main/plugins/searchProviders.ts` 注册），`AppSettings.search.providers` 数组持久化，「测试连接」handler 即时回写测试结果；cloneSettings 走 `normalizeSearchSettings`，与 models / objectStorage 同口径。videoJob 轮询的瞬时网络错误不再判死整条生成任务——`pollOnce` 区分鉴权类（Key 失效 / 无权限，仍立即判死）与其余（超时 / DNS / 5xx / 限流，前 2 次 5s、3-5 次 15s、之后 30s，连续失败满 20 次 ≈ 8 分钟窗口才以「轮询供应商状态连续失败」判死；成功一次即清零计数），下载产物阶段加重试 3 次间隔 5s，DeepSeek agent 收到瞬时错误不再重发 vendor 重复生成。Asset removal 的 `ASSET_REMOVED` 载荷由 `string` 改为 `{ id, path }`，主进程 `ASSET_DELETE` IPC handler 也补广播，渲染端 `ChatPanel` 用源路径 → 资产 id 映射 + 按 id 优先反查（id 缺失按文件名兜底）让对话产物卡的「已保存」状态随删除自动回退；MCP `asset_delete` 路径同步广播新载荷。撤销 `package.json` 的 `"type": "module"`——Node 24 看到这个字段会把项目按 ESM 加载，所有 named import 第三方 CJS 包（electron-updater / @electron-toolkit/utils / electron 等 196+ 处）立刻抛 `SyntaxError: Named export … not found`；保留每次 spawn 一次的 .ts reparse 性能开销换应用可启动。CI 把 `deepseekHarnessFailure.ts:10` 的余额不足正则与 `generateText.ts:136` 的 LLM lead 提示词补进 `check-hardcoded-cjk` 的 `EXEMPT_FILES`，与既有 dsh / LLM 提示词豁免粒度一致。

## [6.2.2] — 2026-09-15

6.2.2 版本：把对话产物的「自动入库」从根上彻底封掉——`asset_import` 收口为「拒收工程内任意素材」（不再仅拦 `Cache/` / `Output/`，连 `Assets/` 已入库资产也拒）；五个生成工具的 `inputSchema` 一律去掉 `outputDir` / `folderId` 并改走 Cache-only，handler 不再传 settle 钩子，`cacheOnlyGenExtraParams` 加抹顶层字段兜底 harness 缓存旧 schema 的字段泄漏；对话流按内容指纹（文件总字节数 + 前 8KB 的 djb2）对近 20 张卡去重兜底继续保留。资产库补多选文件 / 多选目录拖拽（`folder_move` IPC + MCP：磁盘搬移 + descendant asset.relativePath 写回 + 成环检测 + 同名追加 " 2" / " 3"），右侧 `.card.folder` 与左侧目录树加 draggable 对齐；dsh 异常退出时识别 DeepSeek 余额不足并给充值指路；Graph 右键菜单新增「影视」父级分组（剧本 / 剧集 / 世界元素 / 场 收进二级菜单），`executeGameSystemGenerateNode` 支持把 `params.text` 底稿并入 prompt。HTTP 层把 axios 网络错诊断（`err.code` + `err.cause.code` / `err.cause.message`）从 Kling 抽到通用 `readHttpError`，覆盖全部 33 个 provider 适配器——DNS / TCP / TLS 握手前 socket 被断之类的网络错，UI 端现在能直接看到 `ECONNRESET` / `UND_ERR_SOCKET` 这种稳定标签，便于一眼判断是防火墙断握手还是连接被拒。CI 工作流最后一批 `node20` action 升到 Node 24，`release.yml` 跑通。

## [6.2.1] — 2026-09-13

6.2.1 版本：修掉两处「界面上看着没做成」的缺陷——SVG 资产在 `@` 引用选择器与对话引用 chip 里始终没有预览（预览取的是「规划中的位图缩略图」路径，而矢量图根本不生成位图缩略图、那个路径永不落盘），以及把 AI 对话（或任意编辑器页签）拖到资产 / 参数上松手后落点半屏灰框残留在面板上（dockview 对同一个 dock 内的面板拖动只触发 `onDidMovePanel`，落点被拒时两个事件都不触发，锚点覆盖层无人清理）；另把 `@` 引用选择器的 SVG 矢量图单列一类（页签变为「全部 / 图片 / GIF / SVG / 视频 / 音频」）。
